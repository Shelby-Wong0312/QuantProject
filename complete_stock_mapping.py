...
class CapitalToYahooMapper:
    ...
    def clean_symbol(self, symbol):
        ...
        # 移除貨幣後綴
        currency_suffixes = ['USD', 'GBP', 'EUR', 'JPY', 'CHF', 'CAD', 'AUD', 'SGD', 'HKD', 'CNH', 'CNY', 'DKK', 'SEK', 'NOK']
    ...
    def generate_yahoo_candidates(self, capital_stock):
        ...
        # 4. 加密貨幣處理
        crypto_keywords = ['BTC', 'ETH', 'DOGE', 'ADA', 'DOT', 'LINK', 'UNI', 'MATIC', 'SOL', 
                          'AVAX', 'SHIB', 'XRP', 'BNB', 'ATOM', 'LTC', 'TRX', 'ALGO', 'XLM',
                          'VET', 'FIL', 'EOS', 'AAVE', 'MKR', 'COMP', 'SNX', 'YFI', 'SUSHI']
    ...
    def map_all_stocks(self):
        ...
        for i in tqdm(range(0, len(stocks_to_process), batch_size), desc="Processing batches"):
            ...
            if (i + batch_size) % 100 == 0:
                ...
                print(f"[STATS] Mapped: {len(self.mapped_stocks)}, Failed: {len(self.unmapped_stocks)}")
                time.sleep(1)  # 避免請求過快
    ...
    def save_final_results(self):
        ...
        full_results = {
            ...
            'statistics': {
                ...
                'success_rate': f"{len(self.mapped_stocks) / len(self.capital_stocks) * 100:.1f}%" if self.capital_stocks else "0%"
            },
...
def main():
    ...
    for stock in mapper.mapped_stocks[:10]:
        print(f"  {stock['capital_ticker']:10} -> {stock['yahoo_symbol']:10} : {stock['capital_name'][:30]}")
