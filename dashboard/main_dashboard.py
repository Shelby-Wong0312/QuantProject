...
def create_heatmap(positions):
    ...
    for symbol in symbols:
        ...
        data.append([pos["quantity"], pos["avg_price"], pos["current_price"], pnl])
...
def main():
    ...
    st.dataframe(
        ...
        use_container_width=True,
        hide_index=True,
        column_config={"side": st.column_config.TextColumn("Side", help="Buy or Sell")},
    )
    ...
    with st.expander("🚨 Risk Alerts", expanded=False):
        ...
        if abs(risk_metrics["max_drawdown"]) > 0.1:
            st.warning(
                f"📉 Maximum drawdown exceeded threshold: {risk_metrics['max_drawdown']:.2%}"
            )
