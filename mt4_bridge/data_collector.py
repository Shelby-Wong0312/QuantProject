...
class DataStorage:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # 創建索引
            conn.execute("CREATE INDEX IF NOT EXISTS idx_tick_symbol_timestamp ON tick_data(symbol, timestamp)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_ohlc_symbol_timestamp ON ohlc_data(symbol, timestamp, timeframe)")
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (ohlc.symbol, ohlc.timestamp, ohlc.timeframe.value,
                      ohlc.open, ohlc.high, ohlc.low, ohlc.close, ohlc.volume, ohlc.tick_count))
        except Exception as e:
            logger.error(f"保存OHLC數據失敗: {e}")
    
    def get_tick_data(self, symbol: str, start_time: datetime = None, 
                      end_time: datetime = None, limit: int = None) -> List[TickData]:
        """獲取Tick數據"""
        try:
            with sqlite3.connect(self.db_path) as conn:
    ...
    def get_ohlc_data(self, symbol: str, timeframe: TimeFrame, 
                      start_time: datetime = None, end_time: datetime = None, 
                      limit: int = None) -> List[OHLCData]:
        """獲取OHLC數據"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                query = """
...
class MT4DataCollector:
    ...
    def add_ohlc_callback(self, callback: Callable[[OHLCData], None], 
                         symbols: List[str] = None, timeframes: List[TimeFrame] = None):
        """
        添加OHLC數據回調函數
    ...
    def get_latest_data(self, symbol: str, timeframe: TimeFrame, limit: int = 100) -> List[OHLCData]:
        """獲取最新的歷史數據"""
    ...
    def calculate_indicators(self, symbol: str, timeframe: TimeFrame, 
                           indicator: str, **params) -> List[float]:
        """
        計算技術指標
