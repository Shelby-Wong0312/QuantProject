...
class OHLCAggregator:
    ...
    def __init__(self, 
        ...
        logger.info(f"OHLC聚合器已初始化，品種: {symbols}, 時間框架: {[tf.value for tf in self.timeframes]}")
            ...
            for timeframe in self.timeframes:
                ...
                if current_bar is None or current_bar.timestamp != bar_start_time:
                    ...
                    if current_bar is not None:
                        ...
                        logger.debug(f"完成K線: {current_bar.symbol} {current_bar.timeframe.value} "
    ...
    def get_ohlc_dataframe(self, symbol: str, timeframe: TimeFrame, count: int = None) -> pd.DataFrame:
        ...
        with self._lock:
            ...
            if not bars:
                return pd.DataFrame(columns=['timestamp', 'open', 'high', 'low', 'close', 'volume', 'tick_count'])
    ...
    def get_indicators_dataframe(self, symbol: str, timeframe: TimeFrame, count: int = None) -> pd.DataFrame:
        """獲取技術指標數據的 DataFrame"""
        if (not self.enable_indicators or 
            symbol not in self.indicators or 
            timeframe not in self.indicators[symbol]):
            return pd.DataFrame()
    ...
    def get_latest_bar(self, symbol: str, timeframe: TimeFrame) -> Optional[OHLCBar]:
        """獲取最新完成的K線"""
        if (symbol not in self.ohlc_data or 
            timeframe not in self.ohlc_data[symbol] or
            not self.ohlc_data[symbol][timeframe]):
            return None
    ...
    def get_current_bar(self, symbol: str, timeframe: TimeFrame) -> Optional[OHLCBar]:
        """獲取當前未完成的K線"""
        if (symbol not in self.current_bars or 
            timeframe not in self.current_bars[symbol]):
            return None
...
async def example_usage():
    ...
    def on_bar_complete(bar: OHLCBar):
        print(f"K線完成: {bar.symbol} {bar.timeframe.value} "
