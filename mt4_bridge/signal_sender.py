...
class RiskManager:
    ...
    def validate_signal(self, signal: TradingSignal, account_balance: float,
                       current_positions: List[PositionInfo]) -> tuple[bool, str]:
        """
        驗證交易信號是否符合風險管理規則
        ...
        try:
            ...
            if symbol_positions >= self.max_positions_per_symbol:
                return False, f"品種 {signal.symbol} 已達到最大持倉限制: {self.max_positions_per_symbol}"
            ...
            if signal.risk_percent > self.max_risk_per_trade * 100:
                return False, f"單筆交易風險過高: {signal.risk_percent}% > {self.max_risk_per_trade * 100}%"
            ...
            if risk_amount > account_balance * self.max_risk_per_trade:
                return False, f"風險金額過高: {risk_amount} > {account_balance * self.max_risk_per_trade}"
...
class MT4SignalSender:
        ...
        try:
            ...
            if response and response.get("success"):
                ...
                result = OrderResult(
                    ...
                    error_message=response.get("error_message", "未知錯誤") if response else "連接超時"
                )
    ...
    def _send_exit_order(self, signal: TradingSignal) -> OrderResult:
        ...
        try:
            ...
            close_data = {
                ...
                "volume": min(signal.volume, position.volume) if signal.volume > 0 else position.volume,
                "price": signal.price,
                "max_slippage": signal.max_slippage,
                "signal_id": signal.signal_id
            }
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        success_rate = (self.orders_filled / self.signals_sent * 100) if self.signals_sent > 0 else 0
...
# 便利函數
def create_entry_signal(symbol: str, order_type: OrderType, volume: float,
                       price: float = 0.0, stop_loss: float = 0.0, 
                       take_profit: float = 0.0, **kwargs) -> TradingSignal:
    """創建進場信號"""
...
def create_exit_signal(symbol: str, volume: float = 0.0, price: float = 0.0, 
