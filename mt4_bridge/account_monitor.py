...
class AccountMonitorStorage:
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # 創建索引
            conn.execute("CREATE INDEX IF NOT EXISTS idx_snapshots_timestamp ON account_snapshots(timestamp)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_alerts_timestamp ON alerts(timestamp, level)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_trades_close_time ON trade_history(close_time)")
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    snapshot.timestamp, snapshot.balance, snapshot.equity,
                    snapshot.margin, snapshot.free_margin, snapshot.margin_level,
                    snapshot.profit, snapshot.drawdown, snapshot.drawdown_percent,
                    snapshot.daily_pnl, snapshot.daily_pnl_percent
                ))
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                """, (
                    ...
                    alert.type.value, alert.message,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    trade.ticket, trade.symbol, trade.order_type.value, trade.volume,
                    trade.open_price, trade.close_price, trade.profit, trade.commission,
                    trade.swap, trade.open_time, trade.close_time, trade.comment,
                    trade.magic_number
                ))
        except Exception as e:
...
class PerformanceAnalyzer:
    ...
    def analyze_trades(trades: List[TradeHistory]) -> Dict[str, Any]:
        ...
        avg_win = sum(t.net_profit for t in winning_trades) / len(winning_trades) if winning_trades else 0
...
class MT4AccountMonitor:
    ...
    def _check_account_status(self):
        ...
        try:
            ...
            # 檢查餘額警報
            if self.initial_balance and balance < self.initial_balance * self.alert_thresholds['low_balance_ratio']:
                self._create_alert(
                    AlertLevel.WARNING,
            ...
            if self.last_snapshot:
                ...
                if time_diff.total_seconds() >= 86400:  # 24小時
                    daily_pnl_percent = (equity - self.last_snapshot.equity) / self.last_snapshot.equity * 100
            ...
            if self.last_snapshot:
                ...
                if time_diff >= 86400:  # 24小時
                    if self.last_snapshot.equity > 0:
                        ...
                        snapshot.daily_pnl_percent = snapshot.daily_pnl / self.last_snapshot.equity * 100
    ...
    def _create_alert(self, level: AlertLevel, alert_type: AlertType, message: str, data: Dict[str, Any] = None):
        """創建警報"""
        try:
            alert = Alert(
