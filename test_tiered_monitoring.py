...
class TieredMonitoringTest:
    ...
    def _reallocate_stocks_for_test(self, symbols: List[str]):
        ...
        logger.info(
            f"Reallocated stocks: S={len(self.monitor.tier_stocks[TierLevel.S_TIER])}, "
    ...
    def test_system_startup(self) -> Dict:
        ...
        logger.info(
            f"System startup completed in {startup_time:.2f}s with {status['active_threads']} threads"
        )
        return result
    ...
    def test_monitoring_performance(self, duration_minutes: int = 5) -> Dict:
        ...
        result = {
            ...
            "average_scans_per_minute": (
                sum(monitoring_stats["scans_per_minute"])
                / len(monitoring_stats["scans_per_minute"])
                if monitoring_stats["scans_per_minute"]
                else 0
            ),
            "average_signals_per_minute": (
                sum(monitoring_stats["signals_per_minute"])
                / len(monitoring_stats["signals_per_minute"])
                if monitoring_stats["signals_per_minute"]
                else 0
            ),
    ...
    def test_memory_usage(self) -> Dict:
        ...
        logger.info(
            f"Memory usage - RSS: {result['memory_rss_mb']:.1f}MB, "
    ...
    def _evaluate_4k_capability(self, test_results: Dict) -> Dict:
        ...
        try:
            ...
            # 檢查監控性能
            avg_scans_per_min = test_results.get("performance", {}).get(
                "average_scans_per_minute", 0
            )
