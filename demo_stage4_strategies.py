...
def main():
    ...
    for symbol in test_symbols:
        ...
        print(f"  {symbol}: {len(data)} days data, price range ${data['close'].min():.2f} - ${data['close'].max():.2f}")
    ...
    for strategy, name in all_strategies:
        try:
            ...
            if signals:
                ...
                print(f"{name[:30]:30} | Signals: {len(signals):2} | Buy: {buy_signals:2} | Sell: {sell_signals:2} | Avg Strength: {avg_strength:.2f}")
            else:
