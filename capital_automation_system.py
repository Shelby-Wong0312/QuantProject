...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('capital_automation.log'),
...
class CapitalAutomationSystem:
            ...
            try:
                ...
                if positions:
                    ...
                    for pos in positions:
                        ...
                        if pnl_pct > 5:  # 盈利超過5%
                            logger.info(f"Position {epic} profit {pnl_pct:.2f}% - Consider taking profit")
                            # 可以實現自動平倉邏輯
        ...
        while self.running:
            try:
                ...
                if current_positions >= self.config['max_positions']:
                    logger.info(f"Max positions reached ({current_positions}/{self.config['max_positions']})")
                else:
                    # 掃描市場機會
    ...
    def execute_trade(self, epic: str, signal: Dict):
        ...
        try:
            ...
            # 執行訂單
            logger.info(f"Executing {signal['signal']} on {epic} - Size: {size}, Confidence: {signal['confidence']:.1%}")
            ...
            if result['success']:
                ...
                logger.info(f"Trade executed successfully - Deal: {result['data'].get('dealReference')}")
            else:
    ...
    def generate_report(self):
        ...
        while self.running:
            try:
                ...
                pnl_pct = (pnl / self.performance['start_balance']) * 100 if self.performance['start_balance'] > 0 else 0
