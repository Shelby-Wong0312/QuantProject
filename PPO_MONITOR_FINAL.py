...
class PPOMonitor:
    ...
    def display_signals(self, results):
        ...
        if buy_signals:
            ...
            print("-" * 100)
            print(
                f"{'Symbol':<10} {'Price':>10} {'Change%':>10} {'Confidence':>12} {'Buy%':>8} {'Hold%':>8} {'Sell%':>8}"
            )
            print("-" * 100)
        ...
        print("\n" + "=" * 100)
        print(
            f"SUMMARY: Total={len(results)} | Buy={len(buy_signals)} | Hold={len(hold_signals)} | Sell={len(sell_signals)}"
        )
        ...
        if results:
            ...
            print(
                f"Strongest: {strongest['symbol']} - {strongest['signal']} ({strongest['confidence']*100:.1f}%)"
            )
