...
from quantproject.signals.signal_generator import SignalGenerator, TradingSignal
from quantproject.rl_trading import TradingEnvironment
from quantproject.rl_trading import PPOTrainer
from quantproject.data.data_manager import DataManager
from quantproject.api.capital_client import CapitalComClient, Environment, Order, OrderSide, OrderType
...
class IntegratedTradingSystem:
    ...
    def __init__(self, config: SystemConfig):
        ...
        self.risk_manager = RiskManager(
            max_drawdown=config.risk_limit,
            position_limit=config.max_positions
        )
    ...
    async def _initialize_capital_client(self):
        """初始化 Capital.com 客戶端"""
        # 從安全配置讀取憑證
        from quantproject.security.secure_config import SecureConfig
        config = SecureConfig()
            ...
            for symbol in active_symbols:
                ...
                if stock_data is not None and len(stock_data) > 100:
                    ...
                    if signal.strength > 70 and signal.confidence > 0.6:
                        ...
                        logger.info(f"Strong signal for {symbol}: {signal.action} ({signal.strength:.0f})")
    ...
    def combine_strategies(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Strategy combination complete - {len(decisions['trading_signals'])} signals")
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'total_return': (self.state.portfolio_value - self.config.paper_balance) / self.config.paper_balance,
            ...
            'top_positions': sorted(
                self.state.current_positions.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5]
        }
...
class RiskManager:
    ...
    def evaluate_portfolio(self, 
        ...
        if positions:
            ...
            metrics['concentration_risk'] = max_position / sum(position_values) if sum(position_values) > 0 else 0
