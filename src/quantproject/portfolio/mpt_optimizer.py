...
class MPTOptimizer:
    ...
    def calculate_statistics(self) -> Tuple[pd.Series, pd.DataFrame]:
        ...
        logger.info(f"Expected returns range: [{expected_returns.min():.2%}, {expected_returns.max():.2%}]")
    ...
    def optimize_portfolio(self, prices: pd.DataFrame,
        ...
        # 約束條件
        constraints = [
            {'type': 'eq', 'fun': lambda x: np.sum(x) - 1}  # 權重總和為 1
        ]
        ...
        # 只保留權重 > 1% 的資產
        significant_weights = {k: v for k, v in optimization_result['weights'].items() 
                              if v > 0.01}
        optimization_result['significant_weights'] = significant_weights
        
        logger.info(f"Optimization complete: Return={opt_return:.2%}, Risk={opt_risk:.2%}, Sharpe={opt_sharpe:.2f}")
        logger.info(f"Top holdings: {list(significant_weights.keys())[:5]}")
        
        return optimization_result
    
    def calculate_efficient_frontier(self, prices: pd.DataFrame,
    ...
    def select_uncorrelated_assets(self, prices: pd.DataFrame,
                                  max_correlation: float = 0.7,
                                  min_assets: int = 10) -> List[str]:
        """
        選擇低相關性資產
    ...
    def backtest_strategy(self, prices: pd.DataFrame,
