...
class DWXDataCollector:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"端口配置: PUSH={self.push_port}, PULL={self.pull_port}, SUB={self.sub_port}")
    ...
    def place_order(self, symbol: str, order_type: str, lots: float,
                   sl: float = 0, tp: float = 0, comment: str = "") -> bool:
        """下單"""
        try:
            order = {
                '_symbol': symbol,
...
def test_dwx_connection():
    ...
    if collector.connect():
        ...
        def on_tick(symbol, data):
            print(f"{symbol}: Bid={data['bid']:.5f}, Ask={data['ask']:.5f}, Spread={data['spread']}")
