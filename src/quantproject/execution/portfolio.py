...
from quantproject.core.event import SignalEvent, FillEvent, OrderEvent, EventType
...
class Portfolio:
    ...
    async def on_signal(self, signal: SignalEvent):
        ...
        logger.info(f"Portfolio received signal: {signal.symbol} {signal.direction} {signal.quantity}")
    ...
    async def on_fill(self, fill: FillEvent):
        ...
        logger.info(f"Portfolio received fill: {fill.symbol} {fill.direction} {fill.quantity} @ {fill.fill_price}")
        ...
        logger.info(f"Portfolio Update - Cash: {self.cash:.2f}, Total Value: {total_value:.2f}, PnL: {pnl:.2f} ({pnl_pct:.2f}%)")
