...
from quantproject.core.event import OrderEvent, FillEvent
...
class Broker:
        ...
        async with self.session_lock:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"[Broker] Login failed with status {response.status_code}: {response.text}")
                    return False
    ...
    async def _simulate_fill(self, order: OrderEvent):
        ...
        logger.info(f"[Broker] Order filled (simulated): {fill.symbol} {fill.direction} {fill.quantity} @ {fill.fill_price}")
