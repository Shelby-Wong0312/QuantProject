...
class CalculationConfig:
    ...
    def __post_init__(self):
        ...
        if self.indicators is None:
            self.indicators = [
                'SMA_20', 'EMA_20', 'WMA_20', 'VWAP',
                'RSI_14', 'MACD', 'Stochastic', 'WilliamsR_14', 'CCI_20',
                'BollingerBands', 'ATR_14', 'KeltnerChannel', 'DonchianChannel',
                'OBV', 'VolumeSMA_20', 'MFI_14', 'ADLine'
            ]
...
class IndicatorCalculator:
    ...
    def calculate_all_indicators(self, 
                               ...
                               timeframes: Optional[List[str]] = None) -> Dict[str, Dict[str, Dict[str, pd.DataFrame]]]:
        """
        批量計算所有股票的所有指標
        ...
        logger.info(f"Starting calculation for {len(stocks_data)} stocks across {len(timeframes)} timeframes")
        
        # 分批處理股票數據
        stock_symbols = list(stocks_data.keys())
        batches = [stock_symbols[i:i + self.config.batch_size] 
        ...
        logger.info(f"Cache hit rate: {self.stats['cache_hits']/max(1, self.stats['total_calculations'])*100:.1f}%")
        
        return results
    
    def _calculate_multiprocess(self, 
                               stocks_data: Dict[str, pd.DataFrame],
                               timeframes: List[str],
                               batches: List[List[str]]) -> Dict:
        """多進程批量計算"""
        logger.info(f"Using multiprocessing with {self.config.max_workers} workers")
        
        # 準備參數
        calc_func = partial(self._calculate_batch_worker, 
    ...
    @staticmethod
    def _calculate_batch_worker(batch: List[str], 
                               stocks_data: Dict[str, pd.DataFrame],
                               timeframes: List[str],
                               config: CalculationConfig) -> Dict:
        """工作進程函數"""
        # 重新初始化計算器（避免序列化問題）
    ...
    def _calculate_single_stock(self, 
                               symbol: str,
                               data: pd.DataFrame,
    ...
    def calculate_signals(self, 
                         ...
                         signal_config: Optional[Dict] = None) -> Dict[str, Dict[str, pd.DataFrame]]:
        """
        基於指標數據生成交易信號
    ...
    def _calculate_timeframe_signals(self, 
        ...
        if 'MACD' in indicators:
            ...
            if isinstance(macd_data, pd.DataFrame):
                if 'macd' in macd_data.columns and 'signal' in macd_data.columns:
                    ...
                    signals['macd_buy'] = (macd_line > signal_line) & (macd_line.shift(1) <= signal_line.shift(1))
                    signals['macd_sell'] = (macd_line < signal_line) & (macd_line.shift(1) >= signal_line.shift(1))
        ...
        # 最終信號
        signals['signal_strength'] = (signals.get('total_buy_signals', 0) - 
                                    signals.get('total_sell_signals', 0))
        
        signals['final_signal'] = np.where(
            signals['signal_strength'] > 1, 'BUY',
