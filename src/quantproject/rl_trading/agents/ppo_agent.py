...
class ActorCritic(nn.Module):
    """Lightweight actor-critic network kept for backward compatibility."""
    ...
    def __init__(self, obs_dim: int, action_dim: int, hidden_sizes: tuple[int, ...] = (256, 256)):
        ...
        self._policy_head = nn.Linear(input_dim, action_dim)
        self._value_head = nn.Linear(input_dim, 1)
    ...
    def act(self, observation: torch.Tensor) -> torch.Tensor:
        ...
        return torch.distributions.Categorical(logits=logits).probs.argmax(dim=-1)
