...
class PortfolioTradingEnvironment(gym.Env):
    ...
    def _get_asset_features(self, symbol: str) -> List[float]:
        ...
        features.append((current_price - window_data['close'].min()) / 
        ...
        ma_50 = window_data['close'].rolling(50).mean().iloc[-1] if len(window_data) >= 50 else ma_20
        ...
        for symbol in self.symbols:
            ...
            if target_shares != current_shares:
                ...
                if shares_to_trade > 0 and cost > self.portfolio_state.cash:
                    ...
                    affordable_shares = int(self.portfolio_state.cash / 
                ...
                if shares_to_trade != 0:
                    if shares_to_trade > 0:
                        self.portfolio_state.cash -= shares_to_trade * execution_price * (1 + self.transaction_cost)
                    else:
                        self.portfolio_state.cash += abs(shares_to_trade) * execution_price * (1 - self.transaction_cost)
    ...
    def _calculate_reward(self, step_return: float) -> float:
        ...
        weights_squared = sum(w**2 for s, w in self.portfolio_state.weights.items() 
                            if s != 'cash' and w > 0)
        ...
        if len(self.trade_history) > 0:
            recent_trades = [t for t in self.trade_history if t['timestamp'] == self.current_step - 1]
            if recent_trades:
    ...
    def _get_episode_summary(self) -> Dict[str, Any]:
        ...
        total_return = (self.portfolio_state.total_value - self.initial_capital) / self.initial_capital
        
        summary = {
            'total_return': total_return,
            'annualized_return': total_return * (252 / len(self.episode_returns)) if self.episode_returns else 0,
        ...
        summary['win_rate'] = positive_returns / len(self.episode_returns) if self.episode_returns else 0
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            if len(self.episode_returns) > 0:
                ...
                print(f"Total Return: {(self.portfolio_state.total_value/self.initial_capital - 1):.2%}")