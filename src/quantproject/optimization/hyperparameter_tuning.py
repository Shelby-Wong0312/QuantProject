...
class HyperparameterTuner:
    ...
    async def optimize_ml_parameters(self, 
                                     historical_data: Dict[str, pd.DataFrame],
                                     validation_data: Optional[Dict[str, pd.DataFrame]] = None) -> Dict:
        """
        ...
        async def objective(params: Dict) -> float:
            ...
            try:
                ...
                total_weight = (params.get('model_weight_lstm', 0.33) + 
    ...
    def random_search(self, n_iter: int, historical_data: Dict[str, pd.DataFrame]) -> Dict:
        ...
        for i in range(n_iter):
            ...
            for param, bounds in search_space.items():
                if isinstance(bounds, tuple):
                    if param.endswith('learning_rate'):
                        ...
                        params[param] = 10 ** np.random.uniform(np.log10(bounds[0]), np.log10(bounds[1]))
                    elif any(param.endswith(x) for x in ['dim', 'layers', 'estimators', 'depth', 'positions']):
                        # Integer parameters
            ...
            if (i + 1) % 10 == 0:
                logger.info(f"Random search iteration {i+1}/{n_iter}: Best score = {best_score:.4f}")
        
        return best_params
    
    def _evaluate_params_simple(self, params: Dict, historical_data: Dict[str, pd.DataFrame]) -> float:
        """
