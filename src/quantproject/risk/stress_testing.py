...
class StressTesting:
    ...
    def _calculate_var_cvar_for_scenario(self,
                                        scenario: StressScenario,
                                        position_impacts: Dict) -> Tuple[float, float]:
        """
        為特定情境計算VaR和CVaR
    ...
    def stress_test_liquidity(self, 
                            daily_volumes: Dict[str, float]) -> Dict[str, Dict]:
        """
        流動性壓力測試
...
class ExtremeEventSimulator:
    ...
    def _simulate_recovery_path(self,
                               initial_drop: float,
                               recovery_days: int,
...
if __name__ == "__main__":
    ...
    for event_key in ['2008_financial_crisis', '2020_covid_crash']:
        ...
        print(f"\n{result['event_name']}:")
        print(f"  Impact: ${result['portfolio_impact']:,.0f} ({result['portfolio_impact_pct']:.1%})")
