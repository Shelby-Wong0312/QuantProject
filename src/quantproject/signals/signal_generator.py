...
from quantproject.rl_trading.agents.ppo_agent import ActorCritic
from quantproject.rl_trading import TradingEnvironment
from quantproject.models.ml_models import LSTMPricePredictor
from quantproject.indicators.advanced_indicators import AdvancedIndicators
...
class SignalGenerator:
    ...
    def generate_signal(self,
                       data: pd.DataFrame,
                       symbol: str,
                       current_position: float = 0) -> TradingSignal:
        """
        生成交易信號
        ...
        # 2. LSTM 價格預測
        lstm_signal = self._get_lstm_signal(data) if self.lstm_predictor else {'action': 'HOLD', 'score': 50}
    ...
    def _get_momentum_signal(self, data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            returns_20d = (data['close'].iloc[-1] / data['close'].iloc[-20] - 1) if len(data) >= 20 else 0
            
            # 成交量動量
            volume_ratio = data['volume'].iloc[-5:].mean() / data['volume'].iloc[-20:].mean() if len(data) >= 20 else 1
