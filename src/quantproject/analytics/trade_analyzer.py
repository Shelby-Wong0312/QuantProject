...
class TradeAnalyzer:
    ...
    def calculate_returns(self, trades_df: pd.DataFrame, initial_capital: float = 10000) -> pd.DataFrame:
        ...
        trades_df['rolling_sharpe_20'] = trades_df['returns'].rolling(window=20).apply(
    ...
    def calculate_basic_metrics(self, trades_df: pd.DataFrame, initial_capital: float = 10000) -> Dict[str, Any]:
    ...
    def calculate_risk_metrics(self, trades_df: pd.DataFrame, initial_capital: float = 10000) -> Dict[str, Any]:
    ...
    def analyze_trade_patterns(self, trades_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            # Daily performance
            daily_pnl = trades_df.groupby('day_of_week')['pnl'].agg(['count', 'mean', 'sum']).round(2)
            day_names = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
            daily_pnl.index = [day_names[i] for i in daily_pnl.index]
            
            # Symbol analysis
            if 'symbol' in trades_df.columns:
                symbol_performance = trades_df.groupby('symbol')['pnl'].agg(['count', 'mean', 'sum']).round(2)
                ...
                patterns['best_symbol'] = symbol_performance.index[0] if not symbol_performance.empty else None
                patterns['worst_symbol'] = symbol_performance.index[-1] if not symbol_performance.empty else None
            
            # Trade size analysis
            if 'quantity' in trades_df.columns:
                trades_df['trade_size'] = trades_df['quantity'] * trades_df['price']
                size_bins = pd.qcut(trades_df['trade_size'], q=5, labels=['XS', 'S', 'M', 'L', 'XL'])
                size_performance = trades_df.groupby(size_bins)['pnl'].agg(['count', 'mean', 'sum']).round(2)
    ...
    def calculate_monthly_returns(self, trades_df: pd.DataFrame, initial_capital: float = 10000) -> pd.DataFrame:
        ...
        try:
            ...
            monthly_data = trades_df.groupby('year_month').agg({
                'pnl': ['sum', 'count', 'mean'],
                'equity': 'last',
                'returns': 'sum'
            }).round(4)
            ...
            monthly_data.columns = ['monthly_pnl', 'trade_count', 'avg_trade_pnl', 'end_equity', 'monthly_return']
    ...
    def compare_strategies(self, trades_df: pd.DataFrame, strategy_column: str = 'strategy') -> Dict[str, Any]:
    ...
    def generate_comprehensive_analysis(self, 
        ...
        try:
            ...
            analysis['data_summary'] = {
                ...
                'date_range': {
                    'start': trades_df['timestamp'].min().strftime('%Y-%m-%d') if not trades_df.empty else None,
                    'end': trades_df['timestamp'].max().strftime('%Y-%m-%d') if not trades_df.empty else None
                },
