...
class VisualizationCharts:
    ...
    def create_equity_curve(self, 
        ...
        if trades_df.empty:
            fig = go.Figure()
            fig.add_annotation(text="No trade data available", x=0.5, y=0.5, 
    ...
    def create_returns_distribution(self, trades_df: pd.DataFrame, initial_capital: float = 10000) -> go.Figure:
        """Create returns distribution analysis with statistical overlay"""
        
        if trades_df.empty:
            fig = go.Figure()
            fig.add_annotation(text="No return data available", x=0.5, y=0.5,
                             xref="paper", yref="paper", showarrow=False, font_size=16)
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Returns Histogram', 'Q-Q Plot vs Normal', 
        ...
        y_norm = (1/np.sqrt(2*np.pi*returns.var())) * np.exp(-0.5*((x_norm-returns.mean())/returns.std())**2)
        ...
        # Add reference line
        min_val, max_val = min(qq_data[0][0].min(), qq_data[0][1].min()), max(qq_data[0][0].max(), qq_data[0][1].max())
        fig.add_trace(
            go.Scatter(
                x=[min_val, max_val],
                y=[min_val, max_val],
    ...
    def create_correlation_heatmap(self, returns_data: Dict[str, pd.Series]) -> go.Figure:
        ...
        if not returns_data or len(returns_data) < 2:
            fig = go.Figure()
            fig.add_annotation(text="Need at least 2 series for correlation", x=0.5, y=0.5,
                             xref="paper", yref="paper", showarrow=False, font_size=16)
    ...
    def create_trade_scatter(self, trades_df: pd.DataFrame) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('PnL vs Duration', 'PnL vs Trade Size', 
                          'Trade Size Distribution', 'Hourly Trade Pattern'),
            specs=[[{}, {}], [{}, {}]]
        )
        ...
        fig.add_trace(
            go.Bar(
                ...
                y=hourly_pnl.values,
                name='Hourly PnL',
                marker_color=[self.colors['success'] if x >= 0 else self.colors['danger'] for x in hourly_pnl.values],
                opacity=0.8,
                showlegend=False
            ),
            row=2, col=2
    ...
    def create_risk_return_scatter(self, strategies_data: Dict[str, Dict]) -> go.Figure:
        ...
        if not strategies_data:
            fig = go.Figure()
            fig.add_annotation(text="No strategy data available", x=0.5, y=0.5,
                             xref="paper", yref="paper", showarrow=False, font_size=16)
        ...
        if not strategies:
            fig = go.Figure()
            fig.add_annotation(text="Insufficient data for risk-return analysis", x=0.5, y=0.5,
                             xref="paper", yref="paper", showarrow=False, font_size=16)
            fig.update_layout(**self.default_layout, title="Risk-Return Analysis")
            return fig
        ...
        fig.add_annotation(x=avg_vol + (max_vol - avg_vol) * 0.7, y=avg_ret + (max_ret - avg_ret) * 0.7,
                          text="High Risk<br>High Return", showarrow=False, 
                          bgcolor="rgba(0,0,0,0.1)", font=dict(size=10))
        
        fig.add_annotation(x=min_vol + (avg_vol - min_vol) * 0.3, y=avg_ret + (max_ret - avg_ret) * 0.7,
    ...
    def create_performance_comparison(self, strategies_data: Dict[str, Dict]) -> go.Figure:
        ...
        metrics_to_compare = ['total_return', 'sharpe_ratio', 'max_drawdown', 'win_rate', 'profit_factor']
