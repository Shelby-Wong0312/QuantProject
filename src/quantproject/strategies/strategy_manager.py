...
from .strategy_interface import (
    TradingSignal, SignalType, StrategyConfig, StrategyStatus,
    StrategyPerformance, Position
)
...
class StrategyManager:
    ...
    def register_strategy(self, strategy: BaseStrategy) -> bool:
        ...
        with self._lock:
            try:
                ...
                logger.error(f"Error registering strategy {strategy.name if hasattr(strategy, 'name') else 'Unknown'}: {e}")
                self._trigger_callbacks('error_occurred', error=e, strategy=strategy.name if hasattr(strategy, 'name') else 'Unknown')
                return False
    ...
    def execute_all_strategies(self, market_data: pd.DataFrame, 
                             timeout: float = 30.0) -> Dict[str, List[TradingSignal]]:
        """
    ...
    def get_consensus_signal(self, all_signals: Dict[str, List[TradingSignal]], 
                           symbol: str, method: str = 'weighted_vote') -> Optional[TradingSignal]:
        """
    ...
    def _majority_vote_consensus(self, signals: List[TradingSignal], symbol: str) -> Optional[TradingSignal]:
        ...
        avg_strength = total_strength[consensus_type] / vote_counts[consensus_type] if vote_counts[consensus_type] > 0 else 0
        ...
        return TradingSignal(
            ...
            metadata={'method': 'majority_vote', 'vote_count': vote_counts[consensus_type], 'total_strategies': len(signals)}
        )
    
    def _weighted_vote_consensus(self, signals: List[TradingSignal], symbol: str) -> Optional[TradingSignal]:
        ...
        return TradingSignal(
            ...
            timestamp=pd.Timestamp.now(),
            metadata={'method': 'weighted_vote', 'scores': weighted_scores, 'total_strategies': len(signals)}
        )
    
    def _average_strength_consensus(self, signals: List[TradingSignal], symbol: str) -> Optional[TradingSignal]:
        ...
        return TradingSignal(
            ...
            timestamp=pd.Timestamp.now(),
            metadata={'method': 'average_strength', 'averages': avg_strengths, 'total_strategies': len(signals)}
        )
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'avg_execution_time': np.mean(list(self.execution_times.values())) if self.execution_times else 0.0,
