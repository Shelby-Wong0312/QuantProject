...
class MomentumStrategy(BaseStrategy):
    ...
    def _initialize_parameters(self) -> None:
        ...
        self.macd_indicator = MACD(fast_period=self.macd_fast, slow_period=self.macd_slow, signal_period=self.macd_signal)
        
        logger.info(f"{self.name}: Initialized with RSI({self.rsi_period}), "
    ...
    def _calculate_signal_strength(self, rsi: float, macd: float, signal: float, 
...
def create_momentum_strategy(symbols: List[str] = None, 
                           initial_capital: float = 100000) -> MomentumStrategy:
    """
    創建動量策略實例
