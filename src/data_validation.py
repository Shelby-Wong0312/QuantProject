...
class DataValidator:
            ...
            if score >= 0.95:
                ...
                if issues:
                    results["anomalies"].append(
                        {"symbol": symbol, "score": score, "issues": issues}
                    )
    ...
    def _check_price_consistency(self, df: pd.DataFrame) -> List[str]:
        ...
        invalid_open = df[
            (df["open_price"] > df["high_price"]) | (df["open_price"] < df["low_price"])
        ]
    ...
    def generate_quality_report(self) -> str:
        ...
        report.append(
            f"Average Quality Score: {np.mean(list(summary['quality_scores'].values())):.2%}"
        )
        report.append("")
