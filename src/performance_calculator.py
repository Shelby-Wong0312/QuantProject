...
class PerformanceCalculator:
    ...
    def calculate_portfolio_metrics(self, returns_series):
        ...
        sharpe_ratio = np.sqrt(trading_days) * excess_returns.mean() / returns_std if returns_std > 0 else 0
        
        # Sortino Ratio
        downside_returns = returns[returns < 0]
        downside_std = float(downside_returns.std() * np.sqrt(trading_days)) if len(downside_returns) > 0 else 0.01
    ...
    def _generate_synthetic_performance(self):
        ...
        return {
            ...
            'annual_returns': {
                2010: 15.2, 2011: -5.8, 2012: 18.9, 2013: 25.3, 2014: 12.7,
                2015: -2.3, 2016: 14.8, 2017: 21.5, 2018: -8.9, 2019: 28.4,
                2020: 25.5, 2021: 18.3, 2022: -15.2, 2023: 22.1, 2024: 19.8
            },
            'portfolio_returns': strategy_daily_returns.tolist(),  # Full 15 years
            'spy_returns': spy_daily_returns.tolist(),  # Full 15 years
            'dates': pd.date_range(start='2010-08-12', periods=total_days, freq='B').strftime('%Y-%m-%d').tolist()
        }
...
def main():
    ...
    output_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                               'analysis_reports', 'data_exports', 'performance_metrics.json')
