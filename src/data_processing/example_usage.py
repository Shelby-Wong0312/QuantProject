...
def example_feature_engineering():
    ...
    print(
        f"Price features: {len([col for col in price_features.columns if col not in clean_data.columns])}"
    )
    print(
        f"Volume features: {len([col for col in volume_features.columns if col not in clean_data.columns])}"
    )
    print(
        f"Return features: {len([col for col in return_features.columns if col not in clean_data.columns])}"
    )
