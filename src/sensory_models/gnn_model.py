...
class StockCorrelationGNN(nn.Module):
    ...
    def predict_correlation(
        self, node1_embedding: torch.Tensor, node2_embedding: torch.Tensor
    ) -> torch.Tensor:
        """
        預測兩個節點（股票）之間的相關性
...
class TemporalStockGNN(StockCorrelationGNN):
    ...
    def __init__(self, *args, sequence_length: int = 20, **kwargs):
        ...
        self.temporal_attention = nn.MultiheadAttention(
            embed_dim=self.hidden_dim, num_heads=4, dropout=kwargs.get("dropout", 0.1)
        )
...
class GNNFeatureExtractor:
    ...
    def extract_features(self, graph_data: Data) -> Dict[str, np.ndarray]:
        """
        從圖數據中提取特徵
    ...
    def get_correlation_matrix(
        self, node_embeddings: torch.Tensor, symbols: List[str]
    ) -> np.ndarray:
        """
        計算相關性矩陣
