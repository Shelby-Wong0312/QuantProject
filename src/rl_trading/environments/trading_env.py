...
class TradingEnvironment(gym.Env):
    ...
    def __init__(
        ...
        self.action_space = (
            spaces.Discrete(self.action_space_handler.n_actions)
            if action_type == "discrete"
            else spaces.Box(low=-1, high=1, shape=(1,))
        )
    ...
    def _load_data(self):
        ...
        if len(self.data) < self.window_size + self.max_steps:
            raise ValueError(
                f"Insufficient data: need at least {self.window_size + self.max_steps} rows"
            )
    ...
    def _calculate_features(self):
        ...
        self.data["high_low_ratio"] = (self.data["close"] - self.data["low"]) / (
    ...
    def step(self, action: Union[int, float]) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        trade_details = self.action_space_handler.process_action(
            action, self.position, self.cash, current_price, self._get_portfolio_value()
        )
    ...
    def _get_state(self) -> np.ndarray:
        ...
        portfolio_state = {
            "position": self.position,
            "position_pct": (
                self.position * current_data["close"] / self._get_portfolio_value()
                if self._get_portfolio_value() > 0
                else 0
            ),
            "unrealized_pnl": (
                self.position * (current_data["close"] - self.avg_entry_price)
                if self.position > 0
                else 0
            ),
    ...
    def _get_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "portfolio_exposure": (
                abs(self.position * current_data["close"] / self._get_portfolio_value())
                if self._get_portfolio_value() > 0
                else 0
            ),
            "price_trend": (
    ...
    def render(self, mode: str = "human"):
        ...
        if mode == "human":
            ...
            if self.actions_taken:
                ...
                print(
                    f"Last Action: {self.action_space_handler.get_action_meanings()[last_action] if isinstance(last_action, int) else last_action}"
                )
