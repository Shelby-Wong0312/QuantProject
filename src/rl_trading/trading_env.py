...
class TradingEnvironment(gym.Env):
    ...
    def _prepare_features(self):
        ...
        # 選擇特徵
        feature_columns = [
            'returns', 'price_position', 'volume_ratio', 'rsi',
            'volatility', 'macd_diff', 'bb_position'
        ]
        
        self.features = df[feature_columns].fillna(0).values
        
        # 標準化
        self.features = (self.features - np.mean(self.features, axis=0)) / (np.std(self.features, axis=0) + 1e-10)
