...
class Evaluator:
    ...
    def __init__(self, agent, results_dir: Optional[Path] = None):
        """
        ...
        if all_trades:
            ...
            if "step" in trade_df.columns:
                analysis["trade_timing"] = {
                    "avg_steps_between_trades": (
                        np.diff(trade_df["step"]).mean() if len(trade_df) > 1 else 0
                    ),
                    "max_holding_period": (
                        np.diff(trade_df["step"]).max() if len(trade_df) > 1 else 0
                    ),
                }
    ...
    def _calculate_rolling_sharpe(
        self, episode_results: List[Dict], window: int = 5
    ) -> List[float]:
