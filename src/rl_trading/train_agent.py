...
def main(args):
    ...
    test_results = trainer.evaluate(
        env_type="test" if trainer.test_env else "val", n_episodes=20, save_results=True
    )
    ...
    print(
        f"Total training time: {trainer.training_history['training_time']:.2f} seconds"
    )
    print("\nTest Results:")
    ...
    summary_path = (
        Path(args.results_dir) / trainer.experiment_name / "training_summary.json"
    )
    summary = {
        "experiment_name": trainer.experiment_name,
        "config": {"env": env_config.__dict__, "agent": agent_config},
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--symbol", type=str, default="AAPL", help="Stock symbol to trade"
    )
    parser.add_argument(
        "--env-config",
        type=str,
    ...
    parser.add_argument(
        "--initial-capital", type=float, default=10000, help="Initial trading capital"
    )
    parser.add_argument(
        "--episode-length", type=int, default=252, help="Maximum steps per episode"
    )
    ...
    parser.add_argument(
        "--ent-coe", type=float, default=0.01, help="Entropy coefficient"
    )
    ...
    parser.add_argument(
        "--save-freq", type=int, default=50000, help="Model save frequency"
    )
    parser.add_argument(
