...
class Position:
    ...
    def update(self, trade: Trade):
        ...
        if trade.action == "BUY":
            ...
            if self.shares <= 0:
                logger.warning(
                    f"Attempting to sell {trade.shares} shares with position {self.shares}"
                )
                return
            ...
            realized = (
                shares_to_sell * (trade.price - self.avg_entry_price)
                - trade.commission
                - trade.slippage
            )
...
class PortfolioTracker:
        ...
        for symbol, position in self.positions.items():
            if position.shares > 0:
                ...
                summary = {
                    ...
                    "pnl_pct": (
                        (current_price / position.avg_entry_price - 1) * 100
    ...
    def get_performance_metrics(self, current_prices: Dict[str, float]) -> Dict[str, float]:
        ...
        # Calculate total P&L
        total_pnl = sum(
            pos.get_total_pnl(current_prices.get(sym, pos.avg_entry_price))
        ...
        metrics = {
            ...
            / max(1, self.metrics["winning_trades"] + self.metrics["losing_trades"]),
            "max_drawdown": self.metrics["max_drawdown"],
            "current_drawdown": (current_equity - self.metrics["peak_equity"])
