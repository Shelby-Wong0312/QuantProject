...
from mt4_bridge.data_collector import (
    ...
    TickData, OHLCData, TimeFrame,
    DataStorage, OHLCAggregator, TechnicalIndicators
)
...
class DataCache:
    ...
    def __init__(self, 
                 max_tick_cache: int = 10000,
                 max_ohlc_cache: int = 5000):
    ...
    def get_recent_ohlc(self, symbol: str, timeframe: TimeFrame, 
                       count: int = 100) -> List[OHLCData]:
        """獲取最近的OHLC數據"""
        with self.lock:
...
class MT4DataPipeline:
    ...
    def get_dataframe(self, symbol: str, 
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'validity_rate': self.stats['valid_ticks'] / self.stats['total_ticks'] if self.stats['total_ticks'] > 0 else 0
        }
