...
class PerformanceAnalyzer:
    ...
    def calculate_metrics(self, portfolio_values: pd.Series, returns: pd.Series,
        ...
        try:
            ...
            metrics.update(self._calculate_return_metrics(portfolio_values, returns, initial_capital))
    ...
    def _calculate_return_metrics(self, portfolio_values: pd.Series, 
                                 returns: pd.Series, initial_capital: float) -> Dict[str, Any]:
    ...
    def _calculate_drawdown_metrics(self, portfolio_values: pd.Series, 
                                   returns: pd.Series) -> Dict[str, Any]:
        """Calculate drawdown-related metrics"""
        metrics = {}
        
        if len(portfolio_values) == 0:
            return {'max_drawdown': 0, 'max_drawdown_pct': 0, 'avg_drawdown': 0,
                   'drawdown_duration': 0, 'recovery_time': 0}
        ...
        if drawdown_periods:
            ...
            recovery_times = [period['recovery'] for period in drawdown_periods if period['recovery'] > 0]
    ...
    def _calculate_recovery_time(self, drawdowns: pd.Series, recovery_start: int, 
                                peak_value: float) -> int:
    ...
    def _calculate_time_metrics(self, portfolio_values: pd.Series, 
                               returns: pd.Series) -> Dict[str, Any]:
    ...
    def _calculate_benchmark_metrics(self, returns: pd.Series, 
        ...
        if len(aligned_returns) == 0:
            return {'alpha': 0, 'beta': 0, 'correlation': 0, 'tracking_error': 0,
                   'information_ratio': 0}
    ...
    def _get_default_risk_metrics(self) -> Dict[str, Any]:
        ...
        return {
            'volatility': 0, 'volatility_pct': 0,
            'downside_deviation': 0, 'downside_deviation_pct': 0,
    ...
    def _get_default_trading_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'win_rate': 0, 'win_rate_pct': 0, 'total_pnl': 0,
            'avg_trade_pnl': 0, 'median_trade_pnl': 0,
            'avg_win': 0, 'avg_loss': 0, 'largest_win': 0, 'largest_loss': 0,
            'profit_factor': 0, 'risk_reward_ratio': 0,
...
def calculate_performance_metrics(portfolio_values: pd.Series, returns: pd.Series,
