...
class Portfolio:
    ...
    def execute_trade(self, symbol: str, quantity: float, price: float,
                     signal_type: str, timestamp: datetime) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if quantity > 0:  # Buy order
                result.update(self._execute_buy(symbol, quantity, execution_price, 
                                              commission, timestamp, signal_type))
            else:  # Sell order
                result.update(self._execute_sell(symbol, abs(quantity), execution_price, 
                                               commission, timestamp, signal_type))
    ...
    def _execute_buy(self, symbol: str, quantity: float, price: float,
                    commission: float, timestamp: datetime, signal_type: str) -> Dict:
        """Execute buy order"""
    ...
    def _execute_sell(self, symbol: str, quantity: float, price: float,
        ...
        if position.quantity < quantity:
            logger.warning(f"Insufficient shares for {symbol}: need {quantity}, have {position.quantity}")
        ...
        if position.quantity == quantity:
            ...
            logger.debug(f"Partial sale: {symbol} - Sold {quantity}, Remaining: {position.quantity}")
    ...
    def get_positions_summary(self) -> Dict[str, Dict]:
        ...
        for symbol, position in self.positions.items():
            summary[symbol] = {
                ...
                'return_pct': ((position.current_price - position.entry_price) / 
    ...
    def get_portfolio_summary(self) -> Dict[str, Any]:
        ...
        # Calculate exposure
        long_exposure = sum(pos.get_market_value() for pos in self.positions.values() 
                           if pos.is_long())
        short_exposure = sum(pos.get_market_value() for pos in self.positions.values() 
                            if pos.is_short())
    ...
    def close_all_positions(self, market_data: Dict[str, Dict], 
                           timestamp: datetime, reason: str = "Close all") -> List[Dict]:
        """
    ...
    def __repr__(self) -> str:
        ...
        total_value = self.calculate_total_value()
        return (f"Portfolio(value=${total_value:,.2f}, cash=${self.cash:,.2f}, "
