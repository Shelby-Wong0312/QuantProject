...
class MLBacktester:
    ...
    async def backtest_strategy(
        ...
        strategy: Optional[MLStrategyIntegration] = None,
    ) -> BacktestResult:
        """
    ...
    async def _simple_backtest(
        self, historical_data: Dict[str, pd.DataFrame], strategy: MLStrategyIntegration, dates: List
    ) -> Dict:
        """
        ...
        for i, date in enumerate(dates[1:], 1):
            ...
            current_prices = {
                symbol: data["close"].iloc[-1] for symbol, data in current_data.items()
            }
            ...
            if i % 252 == 0:  # Every year
                ...
                logger.info(
                    f"Year {years:.0f}: Portfolio Value=${portfolio_value:,.2f}, "
    ...
    async def _walk_forward_backtest(
        ...
        while window_start + train_days + test_days <= len(dates):
            ...
            logger.info(
                f"Walk-forward window: Train {train_dates[0]} to {train_dates[-1]}, "
    ...
    def _calculate_metrics(self, results: Dict) -> BacktestResult:
        ...
        # Win rate
        winning_trades = sum(1 for t in results["trade_history"] if t.get("pnl", 0) > 0)
        total_trades = len(results["trade_history"])
        win_rate = winning_trades / total_trades if total_trades > 0 else 0

        # Profit factor
        gross_profit = sum(t.get("pnl", 0) for t in results["trade_history"] if t.get("pnl", 0) > 0)
        gross_loss = abs(
            sum(t.get("pnl", 0) for t in results["trade_history"] if t.get("pnl", 0) < 0)
        )
        ...
        # Monthly returns
        monthly_returns = (
            daily_returns.resample("M").apply(lambda x: (1 + x).prod() - 1)
    ...
    def generate_report(
        self, result: BacktestResult, save_path: str = "reports/backtest_report.json"
    ) -> Dict:
        """
        ...
        report = {
            ...
            "trade_statistics": {
                ...
                "avg_trades_per_month": (
                    result.total_trades / (len(result.daily_returns) / 21)
            ...
            "monthly_returns": (
                result.monthly_returns.to_dict() if not result.monthly_returns.empty else {}
            ),
