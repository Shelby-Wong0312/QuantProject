...
class DataQualityMonitor:
            ...
            if col in self.expected_schema:
                ...
                if expected_type == float:
                    try:
                        ...
                        invalid_count = len(col_data) - pd.to_numeric(col_data, errors='coerce').notna().sum()
                        if invalid_count > 0:
                            self.issues.append(DataIssue(
    ...
    def _check_alerts(self, metrics: DataQualityMetrics, symbol: Optional[str] = None):
        ...
        if alerts:
            for alert in alerts:
                logger.warning(f"ALERT: {alert['metric']} = {alert['value']:.2f} < {alert['threshold']:.2f} for {symbol or 'data'}")
