...
class DataPipeline:
    ...
    def __init__(
        ...
        self.technical_indicators = [
            ...
            'BB_upper', 'BB_lower', 'ATR', 'Volume_MA'
        ]
    ...
    def _calculate_microstructure_features(self, df: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            # Price momentum
            features['momentum_1d'] = (df['close'].iloc[-1] / df['close'].iloc[-2] - 1) if len(df) > 1 else 0
            features['momentum_5d'] = (df['close'].iloc[-1] / df['close'].iloc[-5] - 1) if len(df) > 5 else 0
            ...
            features['price_position'] = (
                ...
                (df['high'].rolling(window=20).max().iloc[-1] - df['low'].rolling(window=20).min().iloc[-1])
            ) if len(df) > 20 else 0.5
