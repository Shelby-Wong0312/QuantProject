...
class HealthMonitor:
        ...
        while self.is_running:
            try:
                ...
                full_report = {
                    ...
                    'overall_status': self._determine_overall_status(health_report, resource_report)
                }
    ...
    def check_system_resources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cpu_status = (
                HealthStatus.CRITICAL if cpu_percent > self.resource_thresholds['cpu_percent']
                else HealthStatus.WARNING if cpu_percent > 70
                else HealthStatus.HEALTHY
            )
            
            memory_status = (
                HealthStatus.CRITICAL if memory_percent > self.resource_thresholds['memory_percent']
                else HealthStatus.WARNING if memory_percent > 75
                else HealthStatus.HEALTHY
            )
            
            disk_status = (
                HealthStatus.CRITICAL if disk_percent > self.resource_thresholds['disk_percent']
                else HealthStatus.WARNING if disk_percent > 80
                else HealthStatus.HEALTHY
            )
