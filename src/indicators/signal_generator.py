...
class IndicatorSignalGenerator:
    ...
    def __init__(
        ...
        # 信號權重
        self.weights = {"trend": 0.3, "momentum": 0.3, "volatility": 0.2, "volume": 0.2}
    ...
    def generate_signals(
        self, data: pd.DataFrame, symbol: str, timeframe: str = "1d"
    ) -> List[TradingSignal]:
        """
        生成交易信號
    ...
    def _confirm_signals(
        self, base_signals: List[Dict], indicators: Dict[str, Any], data: pd.DataFrame
    ) -> List[Dict]:
        """確認和過濾信號"""
        if not base_signals:
            return []
    ...
    def _create_trading_signal(
        ...
        # 過濾低質量信號
        if (
            signal_info["strength"] < self.signal_config["min_signal_strength"]
            or confidence < self.signal_config["min_confidence"]
        ):
            return None
    ...
    def _calculate_confidence(
        self, signal_info: Dict, indicators: Dict[str, Any], data: pd.DataFrame
    ) -> float:
        """計算信號信心度"""
        confidence_factors = []
    ...
    def _passes_final_filter(self, signal: TradingSignal, data: pd.DataFrame) -> bool:
        """最終過濾檢查"""
        # 檢查波動率
