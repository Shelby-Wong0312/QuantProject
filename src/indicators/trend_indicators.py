...
class MovingAverageCrossover(BaseIndicator):
    ...
    def calculate(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.ma_type == "SMA":
            ...
            result["fast_ma"] = (
                data["close"]
                .rolling(self.fast_period)
                .apply(lambda x: np.dot(x, weights_fast) / weights_fast.sum(), raw=True)
            )
            # Slow WMA
            weights_slow = np.arange(1, self.slow_period + 1)
            result["slow_ma"] = (
                data["close"]
                .rolling(self.slow_period)
                .apply(lambda x: np.dot(x, weights_slow) / weights_slow.sum(), raw=True)
            )
        else:
