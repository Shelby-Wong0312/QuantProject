...
class MeanReversionStrategy(BaseStrategy):
    ...
    def calculate_signals(self, data: pd.DataFrame) -> List[TradingSignal]:
        ...
        try:
            ...
            # 均值回歸買入信號 (超賣反彈)
            oversold_condition = (
                current_price <= bb_lower  # 觸及下軌
                and current_rsi <= self.rsi_oversold  # RSI超賣
                and current_zscore <= -self.zscore_threshold  # 顯著偏離
                and current_price < ma_short.iloc[-1]  # 價格低於短期均線
            )

            if oversold_condition:
                strength = self._calculate_mean_reversion_strength(
                    current_price, bb_lower, bb_middle, current_rsi, current_zscore, "buy"
                )
            ...
            overbought_condition = (
                current_price >= bb_upper  # 觸及上軌
                and current_rsi >= self.rsi_overbought  # RSI超買
                and current_zscore >= self.zscore_threshold  # 顯著偏離
                and current_price > ma_short.iloc[-1]  # 價格高於短期均線
            )

            if overbought_condition:
                strength = self._calculate_mean_reversion_strength(
                    current_price, bb_upper, bb_middle, current_rsi, current_zscore, "sell"
                )
