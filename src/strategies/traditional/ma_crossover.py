...
class MovingAverageCrossoverStrategy(BaseStrategy):
    ...
    def calculate_signals(self, data: pd.DataFrame) -> List[TradingSignal]:
        ...
        try:
            ...
            # 獲取最新的MA值
            fast_ma_col = (
                fast_ma_data.columns[0]
                if len(fast_ma_data.columns) > 0
                else f"{self.params['ma_type']}_{self.params['fast_period']}"
            )
            slow_ma_col = (
                slow_ma_data.columns[0]
                if len(slow_ma_data.columns) > 0
                else f"{self.params['ma_type']}_{self.params['slow_period']}"
            )
    ...
    def _calculate_signal_strength(
        self, data: pd.DataFrame, signal_type: str, fast_ma: float, slow_ma: float
    ) -> float:
        """
        計算信號強度
            ...
            if self.params["exit_on_reverse_cross"]:
                ...
                for signal in signals:
                    if signal.symbol == position.symbol:
                        # 如果是多頭倉位收到賣出信號
                        if position.size > 0 and signal.signal_type in [
                            SignalType.SELL,
                            ...
                            break
                        # 如果是空頭倉位收到買入信號
                        elif position.size < 0 and signal.signal_type in [
                            SignalType.BUY,
...
if __name__ == "__main__":
    ...
    config = StrategyConfig(
        ...
        parameters={"fast_period": 20, "slow_period": 50, "ma_type": "SMA"},
    )
