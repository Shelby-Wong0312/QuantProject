    ...
    def calculate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(1, len(signals)):
            ...
            if (prev_k <= prev_d and 
                ...
                data['close'].iloc[i] > sma_values.iloc[i] and  # 趨勢過濾
                not position_open):
                
                signals.iloc[i, signals.columns.get_loc('buy')] = True
                signals.iloc[i, signals.columns.get_loc('signal_strength')] = (self.oversold - current_k) / self.oversold
                position_open = True
            ...
            # 死亡交叉賣出（K 向下穿越 D 在超買區）
                ...
                if position_open_time is not None:
                    ...
                    if holding_days >= self.min_holding_period:
                        ...
                        signals.iloc[i, signals.columns.get_loc('signal_strength')] = (current_k - self.overbought) / (100 - self.overbought)
                        position_open = False
                        self.last_signal_time = data.index[i]
            ...
            if i > 20:
                ...
                if (data['low'].iloc[i] <= price_low and 
                    current_k > stoch_low + 5 and
                    not position_open):
                    signals.iloc[i, signals.columns.get_loc('buy')] = True
