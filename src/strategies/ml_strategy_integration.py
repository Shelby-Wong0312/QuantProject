...
from src.ml_models.lstm_price_predictor import LSTMPricePredictor as LSTMAttentionModel
from src.ml_models.xgboost_predictor import XGBoostPredictor as XGBoostEnsemble
...
class MLStrategyIntegration:
    ...
    def __init__(
        ...
        self.lstm_model = LSTMAttentionModel(
            seq_length=60, prediction_horizon=5, hidden_size=128, num_layers=3
        )

        self.xgboost_model = XGBoostEnsemble(prediction_horizon=5, feature_window=60)
        ...
        self.model_weights = {"lstm": 0.35, "xgboost": 0.35, "ppo": 0.30}
    ...
    def extract_features(self, market_data: pd.DataFrame) -> np.ndarray:
        ...
        bb_position = (market_data["close"].iloc[-1] - bb_lower.iloc[-1]) / (
    ...
    async def generate_trading_signals(
        ...
        for symbol, data in market_data.items():
            try:
                ...
                logger.info(
                    f"Signal for {symbol}: {ensemble_signal.action} "
    ...
    async def execute_trades(
        self, signals: List[TradingSignal], trading_simulator: PaperTradingSimulator
    ) -> Dict:
        """
        ...
        for signal in signals:
            try:
                ...
                execution_results["rejected_trades"].append(
                    {"symbol": signal.symbol, "reason": str(e)}
                )
