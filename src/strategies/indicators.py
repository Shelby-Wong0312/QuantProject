...
def add_all_indicators(df: pd.DataFrame, params: dict) -> pd.DataFrame:
    ...
    # KD (Stochastic)
    stoch = ta.stoch(df['High'], df['Low'], df['Close'], k=params['kd_k'], d=params['kd_d'], smooth_k=params['kd_smooth'])
...
def get_candlestick_patterns(df_slice: pd.DataFrame) -> dict:
    ...
    pattern_df = ta.cdl_pattern(df_slice['Open'], df_slice['High'], df_slice['Low'], df_slice['Close'], name="all")
