...
class SignalAggregator:
    ...
    def aggregate_signals(
        self, all_signals: Dict[str, pd.DataFrame], method: str = None
    ) -> pd.DataFrame:
        """
        聚合多個策略的信號
    ...
    def _weighted_voting_consensus(
        ...
        if conflict_mask.any():
            # 選擇分數較高的信號
            consensus.loc[conflict_mask, "buy"] = (
                consensus.loc[conflict_mask, "buy_score"]
                > consensus.loc[conflict_mask, "sell_score"]
            )
