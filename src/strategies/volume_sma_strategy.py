    ...
    def calculate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(self.trend_period, len(signals)):
            ...
            if not self.in_position:
                ...
                if i > 0:
                    prev_volume_high = (
                        data["volume"].iloc[i - 1] > volume_sma_values.iloc[i - 1] * 1.5
                    )
                ...
                if all(conditions_met):
                    ...
                    logger.info(
                        f"Volume surge entry: ratio={volume_ratio:.2f}, price={current_price:.2f}"
                    )
            ...
            # === 賣出條件（長期持有後的退出）===
                ...
                volume_shrink = (
                    volume_ratio
                    < self.entry_volume_ratio * self.optimal_params["exit_volume_ratio"]
                )
