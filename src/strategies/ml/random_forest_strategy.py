...
class RandomForestStrategy(BaseStrategy):
    ...
    def calculate_signals(self, data: pd.DataFrame) -> List[TradingSignal]:
        ...
        try:
            ...
            # 檢查是否需要重訓練
            if self.direction_model is None or len(X) - self.last_retrain > self.retrain_frequency:
    ...
    def _engineer_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            features_df["volume_trend"] = (
                volumes.rolling(window=5).mean() / volumes.rolling(window=20).mean()
            )
            ...
            features_df["volatility_ratio"] = (
                features_df["volatility_5"] / features_df["volatility_20"]
            )
    ...
    def _prepare_training_data(self, features_df: pd.DataFrame, data: pd.DataFrame) -> tuple:
        """
        準備訓練數據
        ...
        direction_labels = np.where(
            ...
            np.where(
                ...
                1,  # 買入
                np.where(
                    future_returns < -0.005, -2, np.where(future_returns < 0, -1, 0)  # 強賣出
                ),
            ),
        )  # 賣出/持有
    ...
    def _train_models(
        self, X: pd.DataFrame, y_direction: np.ndarray, y_strength: np.ndarray
    ) -> None:
        """
        訓練隨機森林模型
...
def create_random_forest_strategy(
