...
class LSTMPredictor(BaseStrategy):
    ...
    def calculate_signals(self, data: pd.DataFrame) -> List[TradingSignal]:
        ...
        try:
            ...
            # 檢查是否需要重訓練
            if len(self.models) == 0 or len(X) - self.last_retrain > self.retrain_frequency:
    ...
    def _prepare_sequences(self, features_df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(self.sequence_length, len(scaled_data) - self.prediction_steps):
            ...
            # 目標：未來收盤價變化率
            current_price = (
                features_df.iloc[i]["close"]
                if "close" in features_df.columns
                else scaled_data[i][0]
            )
            future_price = (
                features_df.iloc[i + self.prediction_steps]["close"]
                if "close" in features_df.columns
                else scaled_data[i + self.prediction_steps][0]
            )
    ...
    def _train_ensemble_models(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        try:
            ...
            for i in range(self.ensemble_models):
                ...
                logger.info(
                    f"{self.name}: Model {i+1} - Train Loss: {train_loss:.6f}, "
    ...
    def _generate_signal_from_prediction(
        self, prediction: float, confidence: float, current_price: float, symbol: str
