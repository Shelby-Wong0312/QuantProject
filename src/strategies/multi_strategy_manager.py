...
class MultiStrategyManager:
    ...
    def __init__(self, max_concurrent_strategies: int = 10):
        ...
        logger.info(
            f"MultiStrategyManager initialized with max {max_concurrent_strategies} concurrent strategies"
        )
    ...
    def execute_all_strategies(
        self, market_data: pd.DataFrame, parallel: bool = True
        ...
        logger.info(
            f"Executed {len(self.active_strategies)} strategies in {execution_time:.2f} seconds"
        )
    ...
    def _voting_consensus(
        self, all_signals: Dict[str, pd.DataFrame], threshold: float
    ) -> pd.DataFrame:
        """
        投票共識機制
