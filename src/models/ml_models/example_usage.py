...
def example_basic_lstm_training():
    ...
    results = trainer.train_model(
        data, test_size=0.2, validation_split=0.1, plot_results=True, save_model=True
    )
...
def example_multi_horizon_prediction():
    ...
    for horizon in horizons:
        ...
        trainer = ModelTrainer(model, preprocessor, output_dir=f"./output/lstm_horizon_{horizon}")

        results = trainer.train_model(
            data, test_size=0.2, validation_split=0.1, plot_results=False, save_model=True
        )

        print(
            f"{horizon}-day prediction - RMSE: {results['test_metrics']['rmse']:.4f}, "
...
def example_feature_extraction():
    ...
    features = extractor.extract_features(data, include_confidence=True, include_trend=True)
...
def example_cross_validation():
    ...
    print(
        f"Mean RMSE: {cv_results['mean_metrics']['rmse']:.4f} "
        f"(±{cv_results['std_metrics']['rmse']:.4f})"
    )
    print(
        f"Mean R²: {cv_results['mean_metrics']['r2']:.4f} "
...
def example_live_prediction():
    ...
    next_predictions = model.predict_next(historical_data, steps=5, preprocessor=preprocessor)
    ...
    print(
        f"Signal: {'BUY' if signal_strength > 0.01 else 'SELL' if signal_strength < -0.01 else 'HOLD'}"
    )
