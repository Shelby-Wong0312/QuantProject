...
class LSTMFeatureExtractor:
    ...
    def train_models(
        ...
        **training_kwargs,
        ...
        for horizon in self.prediction_horizons:
            ...
            # Prepare data
            split_data = preprocessor.create_train_test_split(training_data, test_size=0.2)

            # Train model
            from .model_trainer import ModelTrainer

            trainer = ModelTrainer(
                model, preprocessor, output_dir=f"./models/lstm_horizon_{horizon}"
            )

            train_results = trainer.train_model(training_data, **training_kwargs)
    ...
    def extract_features(
        self, data: pd.DataFrame, include_confidence: bool = True, include_trend: bool = True
    ) -> pd.DataFrame:
        """
        ...
        for horizon, model in self.models.items():
            ...
            try:
                ...
                if include_confidence and confidence is not None:
                    ...
                    features.loc[pred_index, f"{feature_name}_conf_width"] = (
    ...
    def get_live_predictions(
        self, historical_data: pd.DataFrame, current_features: Optional[Dict[str, float]] = None
...
class MultiModelFeatureExtractor:
    ...
    def extract_all_features(self, data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        """
        ...
        for name, extractor in self.extractors.items():
            ...
            try:
                if hasattr(extractor, "extract_features"):
                    ...
                    # Add new features
                    new_cols = [
                        col for col in extractor_features.columns if col not in features.columns
                    ]
