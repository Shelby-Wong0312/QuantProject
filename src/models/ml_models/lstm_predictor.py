...
class LSTMPredictor(BasePredictor):
    ...
    def build_attention_model(self) -> None:
        ...
        attended_representation = layers.Lambda(
            lambda x: tf.reduce_sum(x, axis=1), name="attention_sum"
        ...
        # Create model
        self.model = keras.Model(inputs=inputs, outputs=outputs, name="lstm_attention_predictor")
    ...
    def train(
        ...
        **kwargs,
        ...
        if kwargs.get("use_tensorboard", False):
            callback_list.append(callbacks.TensorBoard(log_dir="./logs", histogram_freq=1))
    ...
    def predict(
        self, X: np.ndarray, return_confidence: bool = False
    ...
    def _calculate_confidence_intervals(
        self, X: np.ndarray, n_iterations: int = 100, confidence_level: float = 0.95
    ) -> np.ndarray:
        """
    ...
    def _calculate_feature_importance(self, X_sample: np.ndarray, y_sample: np.ndarray) -> None:
        """
        Calculate feature importance using gradient-based method
        ...
        if self.config.feature_columns:
            self.feature_importance = {
                col: float(imp) for col, imp in zip(self.config.feature_columns, feature_importance)
            }
        else:
            self.feature_importance = {
    ...
    def predict_next(
        self, historical_data: pd.DataFrame, steps: int = 1, preprocessor: Optional[Any] = None
    ) -> pd.DataFrame:
        """
        ...
        result = pd.DataFrame(
            {"prediction": predictions_original, "prediction_scaled": predictions_scaled},
