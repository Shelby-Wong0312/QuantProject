...
class TimeSeriesPreprocessor:
    ...
    def transform(
        self, data: pd.DataFrame, return_pandas: bool = False
        ...
        if return_pandas:
            ...
            sequence_index = data.index[
                self.sequence_length : len(data) - self.prediction_horizon + 1
            ]

            # Flatten sequences for DataFrame
            X_df = pd.DataFrame(X_sequences.reshape(len(X_sequences), -1), index=sequence_index)
    ...
    def create_train_test_split(
        self, data: pd.DataFrame, test_size: float = 0.2, gap: int = 0
    ) -> Dict[str, Any]:
        """
