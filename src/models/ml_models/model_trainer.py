...
class ModelTrainer:
    ...
    def train_model(
        ...
        split_data = self.preprocessor.create_train_test_split(data_enhanced, test_size=test_size)
        ...
        train_history = self.model.train(X_train, y_train, X_val, y_val, verbose=1)
        ...
        self.training_results = {
            ...
            "actuals": {"train": y_train_orig, "validation": y_val_orig, "test": y_test_orig},
            "indices": {
    ...
    def evaluate_model(
        self, X_test: np.ndarray, y_test: np.ndarray, detailed: bool = True
    ) -> Dict[str, float]:
        """
        ...
        if detailed:
            ...
            if np.any(mask):
                mape = (
                    np.mean(
                        np.abs((y_test_orig[mask] - predictions_orig[mask]) / y_test_orig[mask])
                    )
                    * 100
                )
                metrics["mape"] = mape
    ...
    def cross_validate(self, data: pd.DataFrame, n_splits: int = 5, gap: int = 0) -> Dict[str, Any]:
        """
    ...
    def optimize_hyperparameters(
        ...
        for trial in range(n_trials):
            ...
            # Train model
            try:
                results = self.train_model(data, plot_results=False, save_model=False)
    ...
    def plot_predictions(self) -> None:
        ...
        all_actuals = np.concatenate(
            [self.training_results["actuals"][split] for split in ["train", "validation", "test"]]
        )
    ...
    def _save_training_results(self) -> None:
        ...
        if hasattr(self.model, "feature_importance"):
            importance_df = pd.DataFrame(
                list(self.model.feature_importance.items()), columns=["feature", "importance"]
