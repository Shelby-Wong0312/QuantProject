...
class SentimentDataPipeline:
    ...
    def process_symbols(
        ...
        weighted_scores = self.scorer.calculate_sentiment_scores(
            sentiment_df, method='weighted'
        )
        exponential_scores = self.scorer.calculate_sentiment_scores(
            sentiment_df, method='exponential'
        )
        momentum_scores = self.scorer.calculate_sentiment_scores(
            sentiment_df, method='momentum'
        )
    ...
    def create_sentiment_features(
        ...
        for symbol in symbols:
            if symbol in scores_df['symbol'].values:
                ...
                enhanced_data[f'{symbol}_sentiment_confidence'] = symbol_scores['composite_confidence']
    ...
    def generate_sentiment_report(
        ...
        if 'raw_news' in results:
            ...
            report_lines.append(f"Date range: {news_df['published_date'].min()} to {news_df['published_date'].max()}")
        ...
        if 'signals' in results:
            ...
            for _, signal in signals_df.iterrows():
                report_lines.append(f"{signal['symbol']}: {signal['signal']} "
