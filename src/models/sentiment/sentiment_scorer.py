...
class SentimentScorer:
    ...
    def _weighted_scoring(
        ...
        for group_value, group_df in df.groupby(group_by):
            ...
            time_diffs = (current_time - group_df['published_date']).dt.total_seconds() / 3600  # hours
        ...
        for group_value, group_df in df.groupby(group_by):
            ...
            score_obj = SentimentScore(
                ...
                metadata={
                    'method': 'momentum',
                    'time_span_hours': (
                        group_df['published_date'].max() - 
                        group_df['published_date'].min()
                    ).total_seconds() / 3600
                }
            )
