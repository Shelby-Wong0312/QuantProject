...
class StrategySelector:
    ...
    def rank_strategies_by_composite_score(self, 
    ...
    def select_strategies_by_diversification(self, 
        ...
        for strategy_name, _ in ranked_strategies[1:]:
            ...
            for selected_strategy in selected:
                if (strategy_name in correlation_matrix.index and 
                    selected_strategy in correlation_matrix.columns):
    ...
    def select_strategies_by_machine_learning(self, 
        ...
        logger.info(f"ML model cross-validation score: {cv_scores.mean():.3f} (+/- {cv_scores.std() * 2:.3f})")
        ...
        predictions = rf_classifier.predict_proba(feature_matrix)[:, 1]  # Probability of good performance
    ...
    def optimize_strategy_weights(self, 
        ...
        if method == "equal_weight":
            weights = {strategy: 1.0 / len(self.current_selection) 
    ...
    def select_and_weight_strategies(self, 
                                   correlation_matrix: Optional[pd.DataFrame] = None,
                                   returns_matrix: Optional[pd.DataFrame] = None,
                                   method: str = "composite_score",
                                   weighting_method: str = "inverse_volatility") -> Tuple[List[str], Dict[str, float]]:
        """
    ...
    def _meets_basic_criteria(self, metrics: StrategyMetrics) -> bool:
        ...
        return (
            metrics.sharpe_ratio >= self.criteria.min_sharpe_ratio and
            metrics.annual_return >= self.criteria.min_annual_return and
            metrics.max_drawdown <= self.criteria.max_drawdown and
            metrics.win_rate >= self.criteria.min_win_rate and
            metrics.total_trades >= self.criteria.min_total_trades and
            metrics.volatility <= self.criteria.max_volatility and
    ...
    def _normalize_metrics(self, metrics: StrategyMetrics) -> Dict[str, float]:
        ...
        if sharpe_values:
            ...
            if max_sharpe > min_sharpe:
                normalized['sharpe_ratio'] = (metrics.sharpe_ratio - min_sharpe) / (max_sharpe - min_sharpe)
            else:
        ...
        if return_values:
            ...
            if max_return > min_return:
                normalized['annual_return'] = (metrics.annual_return - min_return) / (max_return - min_return)
            else:
        ...
        if calmar_values:
            ...
            if max_calmar > min_calmar:
                normalized['calmar_ratio'] = (metrics.calmar_ratio - min_calmar) / (max_calmar - min_calmar)
            else:
    ...
    def _prepare_ml_features(self, features: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'annual_return' in features.columns and 'volatility' in features.columns:
            feature_matrix['risk_adjusted_return'] = features['annual_return'] / features['volatility']
    ...
    def evaluate_selection_performance(self, 
        ...
        return {
            ...
            'weight_concentration': max(self.strategy_weights.values()) if self.strategy_weights else 0
        }
    
    def save_selection_results(self, 
                             filepath: str = "reports/strategy_selection_results.json"):
        """
    ...
    def get_selection_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'strategy_weights': self.strategy_weights,
            'selection_criteria': self.criteria.__dict__,
            'last_selection_date': self.selection_history[-1]['timestamp'] if self.selection_history else None
        }
