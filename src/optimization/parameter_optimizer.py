...
class ParameterOptimizer:
    ...
    def grid_search(self, 
        ...
        for i, combination in enumerate(combinations):
            ...
            try:
                ...
                if (i + 1) % 10 == 0:
                    logger.info(f"Grid search progress: {i+1}/{len(combinations)}, "
        ...
        for generation in range(n_generations):
            ...
            for individual in population:
                try:
                    ...
                    fitness_scores.append(float('-inf') if self.direction == "maximize" else float('inf'))
    ...
    def save_optimization_results(self, 
                                 filepath: str = "reports/parameter_optimization_results.json"):
        """
    ...
    def _analyze_convergence(self) -> Dict[str, Any]:
        ...
        return {
            'converged': converged,
            'iterations_to_best': running_best.index(self.best_value) + 1 if self.best_value in running_best else len(running_best),
            'improvement_rate': (running_best[-1] - running_best[0]) / len(running_best) if len(running_best) > 1 else 0
        }


def create_objective_function(backtest_engine: Any, 
                            data: Dict[str, pd.DataFrame],
                            target_metric: str = "sharpe_ratio") -> Callable:
    """
