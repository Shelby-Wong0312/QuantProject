...
class DynamicStopLoss:
    ...
    def __init__(self, 
        ...
        logger.info(f"Dynamic Stop Loss initialized - ATR: {atr_multiplier}x, Trailing: {trailing_percent:.1%}")
        ...
        if stop_info.position_type == PositionType.LONG:
            ...
            if current_price > stop_info.highest_price:
                ...
                if new_stop > stop_info.stop_price:
                    ...
                    logger.debug(f"{symbol}: Trailing stop updated {old_stop:.2f} -> {new_stop:.2f}")
            ...
            for level in sorted(self.profit_lock_levels, reverse=True):
                if profit_pct >= level:
                    ...
                    if lock_price > stop_info.stop_price:
                        ...
                        logger.info(f"{symbol}: Profit lock at {level:.0%} - Stop: {lock_price:.2f}")
                    break
...
class ProfitProtection:
    ...
    def __init__(self, 
                 lock_levels: List[Tuple[float, float]] = None):
        """
        初始化利潤保護
    ...
    def calculate_protection_stop(self, 
                                 entry_price: float,
                                 current_price: float,
                                 position_type: str = 'LONG') -> float:
        """
