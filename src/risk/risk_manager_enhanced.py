...
class EnhancedRiskManager:
    ...
    async def monitor_positions(self, positions: Dict, market_data: Dict) -> List[Dict]:
        """
        實時監控所有持倉
    ...
    async def calculate_risk_metrics(
        self, positions: Dict, market_data: Dict, total_value: float, total_exposure: float
    ) -> RiskMetrics:
        """
        計算風險指標
        ...
        # 計算風險評分
        risk_score = self.calculate_risk_score(leverage, self.current_drawdown, concentration_risk)
    ...
    def calculate_risk_score(self, leverage: float, drawdown: float, concentration: float) -> int:
        """
        計算綜合風險評分
    ...
    async def check_risk_limits(self, metrics: RiskMetrics, signals: List[Dict]):
        """
        檢查風險限制
    ...
    async def log_stop_loss_execution(
        self, symbol: str, position: Dict, exit_price: float, reason: str
    ):
        """
        記錄止損執行
    ...
    async def test_risk_manager():
        ...
        for symbol in positions:
            risk_mgr.stop_loss_manager.set_initial_stop(
                symbol, positions[symbol]["avg_price"], 2.0  # ATR值
            )
