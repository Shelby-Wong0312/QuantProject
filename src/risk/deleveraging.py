...
class RapidDeleveraging:
    ...
    def calculate_portfolio_leverage(
        self, positions: List[Position], account_equity: float
    ) -> float:
        """
    ...
    def create_deleveraging_plan(
        self,
        positions: List[Position],
        account_equity: float,
        strategy: DeleveragingStrategy = DeleveragingStrategy.SMART,
    ) -> DeleveragingPlan:
        """
    ...
    def _calculate_required_reduction(
        self, positions: List[Position], account_equity: float, current_leverage: float
    ) -> float:
        """
    ...
    def _rank_positions(
        self, positions: List[Position], strategy: DeleveragingStrategy
    ...
    def _select_positions_to_close(
        self, ranked_positions: List[Tuple[Position, float]], required_reduction: float
    ) -> List[Dict]:
        """
        Select positions to close
    ...
    def _calculate_risk_reduction(
        self, positions_to_close: List[Dict], all_positions: List[Position]
    ) -> float:
        """
    ...
    def _estimate_new_leverage(
        self, plan: DeleveragingPlan, executed_positions: List[Dict]
    ) -> float:
        """
    ...
    def get_status(self) -> Dict:
        ...
        return {
            ...
            "last_deleveraging": (
                self.deleveraging_history[-1].timestamp.isoformat()
                if self.deleveraging_history
                else None
            ),
        }
