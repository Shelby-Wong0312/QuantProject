...
class SimpleAutoTrader:
    ...
    def load_stocks(self):
        ...
        return ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'AMD', 
                'NFLX', 'BABA', 'JPM', 'BAC', 'WMT', 'DIS', 'V', 'MA', 'PYPL',
                'SQ', 'COIN', 'ROKU', 'SNAP', 'UBER', 'LYFT', 'ZM', 'DOCU']
    ...
    def execute_trade(self, signal):
        ...
        if signal['signal'] == 'BUY' and len(self.positions) < 20:
            ...
            print(f"[BUY] {symbol} @ ${signal['price']:.2f} (RSI:{signal['rsi']:.1f}, Vol:{signal['volume_ratio']:.1f}x)")
    ...
    def scan_market(self):
        ...
        if self.positions:
            print(f"[POSITIONS] Holding {len(self.positions)} positions: {list(self.positions.keys())}")
