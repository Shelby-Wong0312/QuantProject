...
def monitor():
    ...
    try:
        ...
        # Get last 5 trades
        cursor.execute("SELECT timestamp, symbol, action, quantity, price, pnl FROM trades ORDER BY id DESC LIMIT 5")
        ...
        if trades:
            ...
            for trade in trades:
                ...
                print(f"  {timestamp[:16]} | {action:4} {qty:3} {symbol:5} @ ${price:.2f} {pnl_str}")
        conn.close()
    except:
