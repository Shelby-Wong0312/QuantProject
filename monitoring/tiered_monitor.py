...
class TieredMonitor:
    ...
    def _initialize_stock_allocation(self):
        ...
        try:
            ...
            for symbol in all_symbols:
                if (symbol not in self.tier_stocks[TierLevel.S_TIER] and 
                    symbol not in self.tier_stocks[TierLevel.A_TIER]):
                    self._add_stock_to_tier(symbol, TierLevel.B_TIER)
                    
        except FileNotFoundError:
            logger.warning("tradeable_stocks.csv not found, using limited B-tier stocks")
            
        logger.info(f"Initialized stock allocation: S={len(self.tier_stocks[TierLevel.S_TIER])}, "
    ...
    def start_monitoring(self):
        ...
        adjustment_thread = threading.Thread(
            target=self._tier_adjustment_loop,
            daemon=True,
            name="TierAdjustment"
        )
    ...
    def _update_tier_scores(self, stock_info: StockTierInfo, signals: List[Signal]):
        ...
        stock_info.promotion_score = min(1.0, stock_info.promotion_score * 0.9 + promotion_factor * 0.1)
        ...
        if stock_info.signal_strength < 0.3:  # 弱信號
            ...
            stock_info.demotion_score = min(1.0, stock_info.demotion_score * 0.9 + demotion_factor * 0.1)
        else:
    ...
    def _apply_promotion_rule(self, rule: Dict) -> int:
        ...
        for symbol in list(self.tier_stocks[from_tier]):
            if symbol in self.stock_tiers:
                ...
                # 檢查條件
                if (stock_info.signal_strength >= conditions.get('signal_strength', 0.8) and
                    stock_info.signal_count >= conditions.get('min_signals', 2)):
        ...
        for symbol, score in candidates[:max_symbols - current_count]:
            ...
            adjustments += 1
            logger.info(f"Promoted {symbol} from {from_tier.value} to {to_tier.value} (score: {score:.2f})")
    ...
    def _apply_demotion_rule(self, rule: Dict) -> int:
        ...
        for symbol in list(self.tier_stocks[from_tier]):
            if symbol in self.stock_tiers:
                ...
                if (inactive_duration >= conditions.get('inactive_duration', 3600) and
                    stock_info.signal_strength <= conditions.get('max_signal_strength', 0.3)):
                    
                    self._move_stock_to_tier(symbol, to_tier)
                    adjustments += 1
                    logger.info(f"Demoted {symbol} from {from_tier.value} to {to_tier.value} "
    ...
    def get_monitoring_status(self) -> Dict:
        ...
        status = {
            ...
            'uptime_seconds': (current_time - self.performance_stats['start_time']).total_seconds() 
                             if self.performance_stats['start_time'] else 0,
            'tier_counts': {
