#!/usr/bin/env python3
"""
PPO Local Model FULL OOS Backtest (2023-2025) - All 4,215 Stocks
...
class BacktestEngine:
    ...
    def __init__(self, model, config: PPOConfig, initial_capital: float = 100000, min_hold_bars: int = 5):
        self.model = model
        self.config = config
        self.initial_capital = initial_capital
        self.min_hold_bars = min_hold_bars  # 最小持有期（K棒數）
        self.data_loader = LocalDataLoader()

    def run_single_stock(self, symbol: str, data: pd.DataFrame) -> Dict:
        """對單個股票執行回測（含最少持有N根K棒約束）"""
        if len(data) < 252:
            return None
        ...
        for i in range(252, len(data)):
            ...
            # 最少持有N根K棒約束
            elif (action == 2 or action == 3) and position > 0 and hold_bars >= self.min_hold_bars:
    ...
    def calculate_metrics(self, results: List[Dict]) -> Dict:
        ...
        # 正確的 Sharpe 計算：每只股票單獨計算，再取平均
        # （而非對平均 equity curve 計算，那樣會因分散化效應導致 std 虛低）
        per_stock_sharpes = []
        per_stock_sortinos = []
        for r in results:
            if r and "equity_curve" in r:
                ...
                if len(ec) > 1:
                    ...
                    if len(stock_returns) > 0 and np.std(stock_returns) > 0:
                        ...
                        if len(downside) > 0 and np.std(downside) > 0:
                            stock_sortino = np.mean(stock_returns) / np.std(downside) * np.sqrt(252)
                            per_stock_sortinos.append(stock_sortino)
                        else:
                            per_stock_sortinos.append(stock_sharpe)

        sharpe = np.mean(per_stock_sharpes) if per_stock_sharpes else 0
        sortino = np.mean(per_stock_sortinos) if per_stock_sortinos else sharpe
...
def generate_visualizations(metrics: Dict, results: List[Dict], output_dir: str):
    ...
    plt.title(f'PPO Model - Portfolio Equity Curve ({title_suffix})', fontsize=14, fontweight='bold')
...
def run_full_backtest(
    ...
    output_prefix: str = "local_ppo_mixed_oos_full_4215",
    min_hold_bars: int = 5
):
    """執行全量OOS回測"""
    print("\n" + "=" * 80)
    ...
    # 執行回測
    print(f"\n[3/6] Running backtest on {len(stock_data)} stocks (min_hold_bars={min_hold_bars})...")
    engine = BacktestEngine(model, config, min_hold_bars=min_hold_bars)
    results = []
    ...
    # 保存Markdown報告
    # 自動從start_date和end_date提取年份生成文件名
    start_year = start_date[:4]
    end_year = end_date[:4]
    report_filename = f"{output_prefix}_{start_year}_{end_year}.md"
    report_path = os.path.join(output_dir, report_filename)

    with open(report_path, "w", encoding="utf-8") as f:
        f.write(f"# PPO Local Model - FULL OOS Backtest Report ({start_year}-{end_year})\n\n")
    ...
    # 保存JSON數據
    json_filename = f"{output_prefix}_{start_year}_{end_year}_metrics.json"
...
def main():
    ...
    end_date = "2025-08-08"
    output_prefix = "local_ppo_mixed_minhold5_oos_full_4215"
