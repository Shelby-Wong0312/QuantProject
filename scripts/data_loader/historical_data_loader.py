...
class HistoricalDataLoader:
    ...
    async def download_all_symbols_async(self):
        ...
        for i in range(0, len(self.symbols), self.batch_size):
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                futures = {executor.submit(self.download_symbol_data, symbol): symbol 
                          for symbol in batch}
    ...
    def validate_data_quality(self, df: pd.DataFrame, symbol: str) -> DataQualityMetrics:
        ...
        valid_range = ((df['Open'] > 0) & (df['Open'] < 100000) &
                      (df['High'] > 0) & (df['High'] < 100000) &
                      (df['Low'] > 0) & (df['Low'] < 100000) &
                      (df['Close'] > 0) & (df['Close'] < 100000)).sum()
    ...
    def generate_validation_report(self):
        ...
        if self.quality_reports:
            ...
            report['quality_metrics']['symbols_above_threshold'] = sum(1 for s in scores if s >= self.quality_threshold)
            
            # Find best and worst
            sorted_symbols = sorted(self.quality_reports.items(), 
    ...
    async def run_full_pipeline(self):
        ...
        try:
            ...
            logger.info(f"Average quality score: {report['quality_metrics']['average_quality_score']:.2f}")
            logger.info(f"Symbols meeting quality threshold: {report['quality_metrics']['symbols_above_threshold']}")
