    ...
    def run_fast_backtest(self):
        ...
        for idx, symbol in enumerate(all_stocks):
            ...
            if (idx + 1) % 100 == 0:
                ...
                print(
                    f"Progress: {idx+1}/{len(all_stocks)} ({(idx+1)/len(all_stocks)*100:.1f}%) | "
        ...
        for i, row in enumerate(results_df.head(50).itertuples(), 1):
            ...
            print(
                f"{i:<6} {row.symbol:<10} {return_val:>10.2f}% {row.trades:>9} ${row.final_value:>14,.0f}"
            )
        ...
        for row in top_20.itertuples():
            ...
            print(
                f"{row.symbol:<8} - Expected profit: ${expected_profit:>10,.0f} ({return_val:>6.2f}%)"
            )
