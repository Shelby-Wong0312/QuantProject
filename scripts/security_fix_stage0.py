...
def fix_hardcoded_credentials(file_path: Path) -> bool:
    ...
    try:
        ...
        replacements = [
            ...
            (
                r"os\.environ\['CAPITAL_API_KEY'\]\s*=\s*'kugBoHCUcjaaNwGV'",
                "# os.environ['CAPITAL_API_KEY'] removed - use .env file",
            ),
            (
                r"os\.environ\['CAPITAL_IDENTIFIER'\]\s*=\s*'niujinheitaizi@gmail\.com'",
                "# os.environ['CAPITAL_IDENTIFIER'] removed - use .env file",
            ),
            (
                r"os\.environ\['CAPITAL_API_PASSWORD'\]\s*=\s*'@Nickatnyte3'",
