...
async def main():
    ...
    try:
        ...
        print(f"  - Avg Annual Return: {validation_report['portfolio_metrics']['avg_annual_return']:.2%}")
        print(f"  - Avg Sharpe Ratio: {validation_report['portfolio_metrics']['avg_sharpe_ratio']:.2f}")
        ...
        success_criteria = {
            "Data Completeness > 95%": float(data_report['summary']['success_rate'].rstrip('%')) > 95,
            "Data Quality > 90%": data_report['quality_metrics']['average_quality_score'] > 0.90,
            "Annual Return > 15%": validation_report['portfolio_metrics']['avg_annual_return'] > 0.15,
