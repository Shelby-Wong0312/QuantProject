...
class SecurityAuditor:
    ...
    def calculate_security_score(self) -> int:
        ...
        # Scoring system
        high_issues = sum(1 for v in self.results["vulnerabilities"] 
                         if v.get("severity") == "HIGH")
        medium_issues = sum(1 for v in self.results["vulnerabilities"] 
                           if v.get("severity") == "MEDIUM")
        low_issues = sum(1 for v in self.results["vulnerabilities"] 
    ...
    def generate_markdown_report(self, severity_groups: Dict):
        ...
        report.append("|----------|--------|")
        report.append(f"| OWASP Top 10 | {'[WARNING] Review Needed' if self.results['summary']['high_severity'] > 0 else '[OK] Compliant'} |")
        report.append(f"| PCI DSS | {'[WARNING] Review Needed' if self.results['summary']['high_severity'] > 0 else '[OK] Compliant'} |")
        report.append(f"| GDPR | {'[OK] Compliant' if self.results['security_score'] > 70 else '[WARNING] Review Needed'} |")
        report.append("")
        ...
        report.append(f"**Status**: {'PASSED' if self.results['security_score'] >= 85 else 'REQUIRES REMEDIATION'}")
    ...
    def display_summary(self):
        ...
        status = "[PASSED]" if score >= 85 else "[WARNING] NEEDS IMPROVEMENT" if score >= 70 else "[FAILED]"
