...
for idx, row in enumerate(stocks_df.itertuples()):
    ...
    try:
        ...
        annual_return = (((final_value / initial_capital) ** (1/date_range)) - 1) * 100 if date_range > 0 else 0
...
if len(results_df) == 0:
    ...
    print("-" * 80)
    print(f"{'Rank':<5} {'Symbol':<8} {'Total Return':<15} {'Annual Return':<15} {'Years':<8} {'Trades':<8}")
    print("-" * 80)
    
    for i, row in enumerate(results_df.head(20).itertuples(), 1):
        print(f"{i:<5} {row.symbol:<8} {row.total_return:>13.2f}% {row.annual_return:>13.2f}% "
    ...
    print(f"Best performer: {results_df.iloc[0]['symbol']} ({results_df.iloc[0]['total_return']:.2f}%)")
    print(f"Profitable stocks: {len(results_df[results_df['total_return'] > 0])} "
