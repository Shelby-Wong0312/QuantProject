...
class IndicatorBacktest:
    ...
    def run_backtest(self, data: pd.DataFrame, signals: pd.DataFrame) -> Dict:
        ...
        # Annualized return
        years = len(data) / 252
        annualized_return = ((values.iloc[-1] / self.initial_capital) ** (1/years) - 1) * 100 if years > 0 else 0
...
def test_all_indicators_backtest():
    ...
    for symbol in test_symbols:
        ...
        try:
            ...
            best_indicator = max(symbol_results.items(), 
    ...
    sorted_results = sorted(aggregate_results.items(), 
    ...
    print("-" * 80)
    print(f"{'Rank':<5} {'Indicator':<20} {'Avg Return':<12} {'Sharpe':<10} {'Max DD':<10} {'Win Rate':<10} {'Trades':<8}")
    print("-" * 80)
    
    for i, (indicator, metrics) in enumerate(sorted_results, 1):
        print(f"{i:<5} {indicator:<20} {metrics['avg_return']:>10.2f}% {metrics['avg_sharpe']:>9.2f} "
