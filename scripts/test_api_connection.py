...
async def test_authentication():
    ...
    # Create client
    environment = Environment.DEMO if capital_creds.get('environment') == 'demo' else Environment.LIVE
...
async def test_connection(client):
    ...
    if auth_success:
        ...
        print(f"  Security Token: {client.x_security_token[:20]}..." if client.x_security_token else "  No security token")
    else:
    ...
    async def ws_callback(market_data):
        ...
        print(f"  Received: {market_data.symbol} - Bid: {market_data.bid:.4f}, Ask: {market_data.ask:.4f}")
