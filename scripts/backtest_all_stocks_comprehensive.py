...
class ComprehensiveBacktest:
        ...
        try:
            ...
            for i in range(len(df)):
                ...
                if 'buy' in signals.columns and signals['buy'].iloc[i] and portfolio['cash'] > 0:
                    # Buy signal
                    shares_to_buy = int(portfolio['cash'] / (price * (1 + self.commission_rate + self.slippage_rate)))
                    if shares_to_buy > 0:
                ...
                elif 'sell' in signals.columns and signals['sell'].iloc[i] and portfolio['shares'] > 0:
                    # Sell signal
                    revenue = portfolio['shares'] * price * (1 - self.commission_rate - self.slippage_rate)
    ...
    def run_comprehensive_backtest(self):
        ...
        # Save raw results
        results_df.to_csv(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                       'reports', 'all_stocks_backtest_raw.csv'), index=False)
        
        # Analyze by indicator
        print("\n1. INDICATOR PERFORMANCE SUMMARY")
        print("-" * 80)
        print(f"{'Indicator':<15} {'Avg Return':<12} {'Best Return':<12} {'Worst Return':<12} "
        ...
        for ind_name, _ in self.indicators:
            ...
            if len(ind_results) > 0:
                ...
                print(f"{ind_name:<15} {avg_return:>11.2f}% {best_return:>11.2f}% "
        ...
        print(f"{'Rank':<5} {'Symbol':<10} {'Indicator':<15} {'Return':<12} {'Sharpe':<10} "
              f"{'Max DD':<10} {'Win Rate':<10}")
        print("-" * 80)
        
        for i, row in enumerate(top_stocks.itertuples(), 1):
            print(f"{i:<5} {row.symbol:<10} {row.indicator:<15} {row.total_return:>10.2f}% "
        ...
        multi_indicator_stocks = symbol_avg[symbol_avg['positive_indicators'] >= 3].sort_values(
            'total_return', ascending=False).head(20)
        
        print(f"{'Symbol':<10} {'Avg Return':<12} {'Avg Sharpe':<12} {'Avg Win Rate':<12} {'Positive Ind':<12}")
        print("-" * 80)
        
        for symbol, row in multi_indicator_stocks.iterrows():
            print(f"{symbol:<10} {row['total_return']:>11.2f}% {row['sharpe_ratio']:>11.2f} "
        ...
        print(f"Percentage of profitable tests: {(results_df['total_return'] > 0).mean() * 100:.2f}%")
        print(f"Best single result: {results_df['total_return'].max():.2f}% "
        ...
        report_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                  'reports', 'comprehensive_backtest_report.json')
        ...
        for i, (symbol, row) in enumerate(recommended.iterrows(), 1):
            print(f"{i:2}. {symbol:<8} - Avg Return: {row['total_return']:>6.2f}% | "
