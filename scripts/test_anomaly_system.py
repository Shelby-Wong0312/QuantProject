    ...
    async def run_all_tests(self):
        ...
        criteria = {
            "Anomaly Detection Accuracy > 95%": 
                self.test_results['anomaly_detection'].get('accuracy', 0) > 0.95,
            "Circuit Breaker 100% Reliable": 
                self.test_results['circuit_breaker'].get('passed', False),
            "Deleveraging Execution < 1 second": 
                self.test_results['deleveraging'].get('execution_time', 999) < 1.0,
            "All Alerts Sent Correctly": 
                len(self.detector.anomaly_history) > 0,
            "Test Coverage > 90%": 
                success_rate > 0.9
        }
