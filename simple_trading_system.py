...
class SimpleTradingSystem:
    def __init__(self):
        ...
        self.symbols = [
            ...
            'WMT', 'JPM', 'PG', 'MA', 'UNH', 'DIS', 'HD', 'BAC', 'XOM', 'PFE',
            'ABBV', 'KO', 'PEP', 'TMO', 'COST', 'AVGO', 'CSCO', 'NKE', 'MRK', 'CVX',
            'ABT', 'LLY', 'ADBE', 'CRM', 'ORCL', 'ACN', 'VZ', 'NFLX', 'INTC', 'AMD',
            'T', 'WFC', 'MDT', 'UPS', 'MS', 'BMY', 'RTX', 'QCOM', 'NEE', 'HON'
        ]
    ...
    def execute_trade(self, symbol, action, price):
        ...
        if action == 'BUY' and len(self.positions) < self.max_positions:
            ...
            print(f"[SELL] {shares} shares of {symbol} at ${price:.2f} (P&L: ${pnl:.2f}, {pnl_pct:.1f}%)")
            return True
        ...
        if self.positions:
            ...
            for symbol, pos in self.positions.items():
                ...
                if current_price:
                    ...
                    print(f"  {symbol}: {pos['shares']} shares @ ${pos['price']:.2f} | Current: ${current_price:.2f} | P&L: ${pnl:.2f} ({pnl_pct:.1f}%)")
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                print(f"[SCAN] Complete - {signals_generated} signals, {trades_executed} trades executed")
