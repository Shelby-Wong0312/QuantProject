...
from live_trading_system import LiveTradingSystem, Order, Fill, Position, BrokerAdapter
...
class _MyBrokerAdapter(BrokerAdapter):
    ...
    def equity(self) -> float:
        return float(getattr(self.engine, "equity"))

    @property
    def cash(self) -> float:
        return float(getattr(self.engine, "cash"))

    @property
    def upnl(self) -> float:
        return float(getattr(self.engine, "upnl"))

    @property
    def rpnl(self) -> float:
        return float(getattr(self.engine, "rpnl"))
...
def start_background_sync(period_sec: int = 60) -> None:
    ...
    if _lts is None:
        raise RuntimeError("init_lts() not called")
...
def notify_submitted(
    symbol: str, side: str, qty: float, price: float, deal_id: Optional[str] = None
) -> None:
    """下單被接受/送出成功時呼叫。"""
    if _lts is None:
        raise RuntimeError("init_lts() not called")
    _lts.on_order_submitted(
        Order(
            symbol=symbol,
            side=side.lower(),
            quantity=float(qty),
