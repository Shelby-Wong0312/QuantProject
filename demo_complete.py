...
for stock, pred in lstm_predictions.items():
    ...
    print(f"  {stock}: ${pred['current']:.0f} -> ${pred['predicted']:.0f} [{direction} {abs(change):.2%}] (Conf: {pred['confidence']:.0%})")
...
for action in rl_actions:
    if action['action'] != 'HOLD':
        print(f"  [{action['time']}] {action['action']} {action['quantity']} {action['symbol']} (Confidence: {action['confidence']:.0%})")
    else:
        print(f"  [{action['time']}] {action['action']} {action['symbol']} (Confidence: {action['confidence']:.0%})")
...
for sig in signals:
    print(f"  {sig['symbol']}: {sig['signal']} (Strength: {sig['strength']}/100, Risk: {sig['risk']}/100)")
...
for trade in trades_executed:
    ...
    print(f"  [{trade['status']}] {trade['side']} {trade['quantity']} {trade['symbol']} @ ${trade['price']:.2f} (Value: ${value:,.2f})")
...
print(f"Win Rate: {performance['win_rate']:.0%} ({performance['profitable_trades']}/{performance['total_trades']} trades)")
