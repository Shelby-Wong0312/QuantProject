...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("ppo_15years_training.log"), logging.StreamHandler()],
)
...
class SmartDataLoader:
    ...
    def download_stock_data(self, tickers: List[str]):
        ...
        if download_needed:
            ...
            batch_size = 20
            batches = [
                download_needed[i : i + batch_size]
                for i in range(0, len(download_needed), batch_size)
            ]
            ...
            print(
                f"[DOWNLOAD] Successfully downloaded: {success_count}/{len(download_needed)} stocks"
            )
...
def main():
    ...
    # 6. 生成報告
    {
        ...
        "avg_reward_last_50": (
            ...
            if len(trainer.training_history["rewards"]) > 50
            else 0
        ),
