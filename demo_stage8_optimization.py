...
class OptimizationFrameworkDemo:
        ...
        for method_name, method_type in methods:
            ...
            if method_type == "grid_search":
                ...
                results = optimizer.grid_search(
                    strategy=None, param_grid=param_grid, objective_func=grid_objective
                )
            ...
            # Save results
            optimizer.save_optimization_results(f"reports/demo_{method_type}_optimization.json")
    ...
    def demo_strategy_selection(self):
        ...
        selector = StrategySelector(selection_criteria=criteria, max_strategies=5)
        ...
        for method_name, selection_method, weighting_method in methods:
            if selection_method == "diversification":
                ...
                selected, weights = selector.select_and_weight_strategies(
                    method=selection_method, weighting_method=weighting_method
                )
    ...
    async def demo_walk_forward_analysis(self):
        ...
        param_bounds = {"ma_short": (10, 30), "ma_long": (50, 150), "threshold": (0.01, 0.03)}
        ...
        cv_results = analyzer.cross_validation(
            strategy=DemoStrategy(), data=demo_data, param_bounds=param_bounds, folds=3
        )
        ...
        print(
            f"    Probability of Positive Return: {mc_results['probability_positive_return']:.1%}"
        )
        print(
            f"    Sharpe Ratio Mean: {mc_results['simulation_statistics']['sharpe_ratio']['mean']:.3f}"
        )
        ...
        wf_results = analyzer.walk_forward_analysis(
            strategy=DemoStrategy(), data=demo_data, param_bounds=param_bounds
        )
