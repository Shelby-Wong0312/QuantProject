...
class FreeDataClient:
    ...
    def get_batch_quotes(self, symbols: List[str], use_cache: bool = True, 
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_batch = {executor.submit(self._process_batch, batch, i): batch 
        ...
        logger.info(f"Retrieved {len(all_quotes)} quotes, {len(failed_symbols)} failed. Success rate: {success_rate:.1f}%")
                ...
                for symbol, quote_data in quotes.items():
                    conn.execute(
                        """INSERT OR REPLACE INTO real_time_quotes 
                           ...
                           VALUES (?, ?, ?, ?, ?)""",
                        (symbol, quote_data['price'], quote_data['volume'],
                         quote_data['change_percent'], quote_data['timestamp'])
                    )
                conn.commit()
        except Exception as e:
    ...
    def get_market_overview(self) -> Dict:
        ...
        try:
            ...
            market_open = (
                ...
                now.weekday() < 5 and 
                not self._is_market_holiday(now)
            )
...
if __name__ == "__main__":
    ...
    print(f"   Price Range: ${summary.get('price_stats', {}).get('min', 0):.2f} - ${summary.get('price_stats', {}).get('max', 0):.2f}")
