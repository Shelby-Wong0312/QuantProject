...
def drop_unused_assigns(code: str) -> str:
    ...
    pat = re.compile(
        r"^(\s*)(order_id|data|report|best_params|symbols|graph|obs|start_cpu|signals)\s*=\s*(await\s+)?(.+)$",
        re.MULTILINE,
    )
...
def fix_bare_except(code: str) -> str:
    return re.sub(
        r"^(\s*)except\s*:\s*$", r"\1except Exception:", code, flags=re.MULTILINE
    )
...
def normalize_logger_weird_plus(code: str, path: Path) -> str:
    # 通殺 "" + 文字
    code = re.sub(
        r'logger\.(info|warning|error)\(\s*""\s*\+\s*"([^"]*)"\s*\)',
        r'logger.\1(" \2")',
        code,
    )
    # 沒加第二個引號的各種迷因，特例修正：
